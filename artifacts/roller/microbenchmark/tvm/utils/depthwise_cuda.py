...
def generate_depthwise_cuda(source_code, grid, block, N, C, H, W, K, S, P, entry_point):
...
int main() {{
    ...
    int input_size1 = 1 * C * K * K;
    int output_size = N * C * ((H - K + 2 * P) / S + 1) * ((W - K + 2 * P) / S + 1);
    ...
    """.format(source_code, ", ".join(grid), ", ".join(block), N, C, H, W, K, S, P, entry_point)
    return generated
...
def execute(path):
    with open(path, "r") as f:
        ...
        if "ansor" in filename:
            ...
            entry_point = "depthwise_kernel0"
        generated = generate_depthwise_cuda(kernel, launch_config[0:3], launch_config[3:], *param, entry_point)
