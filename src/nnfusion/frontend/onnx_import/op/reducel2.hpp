...
namespace nnfusion
{
    namespace frontend
    {
        namespace onnx_import
        {
            namespace set_1
            {
                NamedNodeVector TranslateReduceL2Op(const onnx::NodeProto& node_proto,
                                                    ...
                                                    std::shared_ptr<nnfusion::graph::Graph> m_graph)
                {
                    ...
                    auto square_op = std::make_shared<op::Square>();
                    auto square_gnode = m_graph->add_node_and_edge(square_op, {input_index});

                    auto sum_op = std::make_shared<op::Sum>(ng_reduction_axes);
                    NamedNodeVector ret;
                    if (keepdims)
                    {
                        auto sum_gnode = m_graph->add_node_and_edge(sum_op, {square_gnode});
                        auto sqrt_op = std::make_shared<op::Sqrt>();
                        auto sqrt_gnode = m_graph->add_node_and_edge(sqrt_op, {sum_gnode});
                        ...
                        nnfusion::AxisVector ng_axis_order(sqrt_gnode->get_shape().size());
                        std::iota(ng_axis_order.begin(), ng_axis_order.end(), 0);
                        auto reshape_op =
                            std::make_shared<op::Reshape>(ng_axis_order, ng_result_shape_with_keep);
                        reshape_op->set_name(node_proto.output(0));
                        auto reshape_gnode = m_graph->add_node_and_edge(reshape_op, {sqrt_gnode});
                    ...
                    else
                    {
                        ...
                        auto sqrt_op = std::make_shared<op::Sqrt>();
                        sqrt_op->set_name(node_proto.output(0));
                        auto sqrt_gnode = m_graph->add_node_and_edge(sqrt_op, {sum_gnode});
                        ret.push_back({node_proto.output(0), sqrt_gnode});
                    }
