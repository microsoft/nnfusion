...
class SimpleCostModel(CostModelBase):
  ...
  def compute_estimate(self, schedule, tile_tensor='output'):
    ...
    compute_workload_thread = self.op.compute_workload(reg_tile_dim,
                                                       tile_tensor)
  ...
  def memory_estimate(self, schedule, mem_level, tile_tensor='output'):
    ...
    for tensor_name in workloads:
      ...
      if mem_transaction_size > 1:
        ...
        tile_subtensor = _fix_reduction_dimension(tile_subtensor,
                                                  reduction_size)
      ...
      memory_latency_tensor = rw_bytes * memory_penalty / self.arch.memory_bw(
          mem_level)
