...
from .cost_model_base import *
from tvm import te
import math
from .global_mem import dram_latency
...
class WarpBasedCostModel(CostModelBase):
  ...
  def block_warps_num(self, reg_tile, smem_tile):
    ...
    return int(
        math.ceil(self.num_tiles(smem_tile, reg_tile) / self.arch._warp_size))

  def get_glbmem_bandwidth(self,
                           schedule,
                           small_glbmem_db,
                           tile_tensor='output'):
    """
  ...
  def compute_estimate(self, schedule, tile_tensor='output'):
    ...
    if self.arch.num_level == 2:
      ...
      compute_kernel_tp = peak_performance * sche_units_num / (
          math.ceil(sche_units_num / sche_units_total) * sche_units_total)

      compute_workload_total = self.op.compute_workload(
          reg_tile, reduction_size,
          tile_tensor) * block_size * grid_size  # / 1000000000 to GFLOPS
  ...
  def memory_estimate(self, schedule, mem_level, tile_tensor='output'):
    ...
    if mem_level == 1 and self.arch.num_level == 2:
      ...
      if block_schedule_unit == 'warp':
        ...
        raise NotImplementedError
      # print(sche_units_num, sche_units_total)
      smem_workload = self.op.memory_workload(reg_tile, reduction_size, 1,
                                              tile_tensor)  # from SMEM to REGS
      smem_workload_total = sum(
          [smem_workload[tensor_name] for tensor_name in smem_workload])

      smem_kernel_tp = peak_performance * sche_units_num / (
          math.ceil(sche_units_num / sche_units_total) * sche_units_total)
      smem_bytes = smem_workload_total * block_size * grid_size
      smem_time_ns = smem_bytes / smem_kernel_tp * 1000000000 / (1024 * 1024 *
                                                                 1024)
      ...
      return DRAM_latency(self.op, schedule.glbmem_bandwidth,
                          self.arch._transaction_size[0] // 4,
                          self.arch._warp_size, reg_tile_dim, smem_tile_dim,
                          reduction_size)
