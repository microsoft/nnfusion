...
_lr_signature = "ID INTEGER FLOAT STRING CHAR WS COMMENT1 COMMENT2 POUND DPOUND SHARED IF GLOBAL SYNC ELSE VOID FOR QUALIFIER TYPEfunction : signature '{' shared_buffer statements '}'signature : GLOBAL VOID ID '(' parameters ')'parameters : parameter\n                  | parameters ',' parameter\n    parameter : type ID\n                 | type QUALIFIER ID\n                 | QUALIFIER type ID\n    type : TYPE \n            | TYPE '*'\n    shared_buffer : shared\n                     | shared_buffer shared\n                     | normal\n    shared : SHARED TYPE ID '[' INTEGER ']' ';' statements : statement\n                  | statements statement\n    statement : sync\n                 | for_loop_static\n                 | normal\n    for_loop_static : FOR '(' assign compare increase ')' '{' statements '}'\n       assign   : TYPE ID '=' INTEGER ';'\n       compare  : ID '<' INTEGER ';'\n       increase : '+' '+' ID\n                | ID '+' '+'\n    sync : SYNC '(' ')' ';' normal : ';'\n               | '{' normal '}'\n    "
