...
def parse(code):
    ...
    # Move output to the last parameter. Need by conv-add
    params_str = re.search("(\(.*?\))", func_sig).group(1)[1:-1]
    params = params_str.split(", ")
    # if (len(params) > 3):
    #     params = params[:2] + params[3:] + [params[2]]
    #     print("reorder parameters to", params)
    #     func_sig = func_sig.replace(params_str, ", ".join(params))
    ...
    func_source = func_sig + func_body

    return func_source, func_sig, func_body, shared_memory, new_code, sync_code, signature[-1]
