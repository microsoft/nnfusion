        ...
        namespace onnx_import
        {
            namespace set_1
            {
                                                  ...
                                                  std::shared_ptr<nnfusion::graph::Graph> m_graph)
                {
                    ...
                    if(mode == "LINEAR")
                    {
                        std::string trans_mode = node.get_attribute_value<std::string>("coordinate_transformation_mode", "pytorch_half_pixel");
                        std::string nearest_mode = node.get_attribute_value<std::string>("nearest_mode", "floor");
                        ...
                        auto resize_op =
                            std::make_shared<nnfusion::op::GenericOp>(node_name, "Resize", op_config);
                        auto resize_gnode = input_indices.size() < 4?
                            m_graph->add_node_and_edge(resize_op, {x_gnode, scales_gnode}) : m_graph->add_node_and_edge(resize_op, {x_gnode, scales_gnode, input_indices[3]});
                        return NamedNodeVector{{node_proto.output(0), resize_gnode}};
                    }
                }
            } // namespace set_1
