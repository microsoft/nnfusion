...
class NNFusionRT:
    def __init__(self, model, inputs, outputs, server = "127.0.0.1:8880", steps=1000):
        self.model = model
        self.inputs = inputs
        self.outputs = outputs
        self.workdir = "./nnfusion/tmp" + str(model.__class__.__name__) + "/"
    ...
    def compile(self, buildall = False, rebuild = False):
        ...
        if buildall:
            torch.onnx.export(self.model, self.inputs, self.workdir + "stencil.onnx", verbose=True,
                              input_names=input_names, output_names=output_names)  # , opset_version=11)
            codegen(os.path.join(self.workdir, "stencil.onnx"),
                    "-f onnx -fkernel_tuning_steps=" + str(self.tuning_step) + " -fantares_mode=1 -fantares_codegen_server=" + self.codegen_server + " -fextern_result_memory=1 -fir_based_fusion=1 -fkernel_fusion_level=0 -fblockfusion_level=0", self.workdir)
    ...
    def run(self, input, output):
        ...
        if isinstance(input, list):
            ...
            for i in range(len(input)):
                in_dict[self.executor.get_inputs()[i].name] = cast_pytorch_tensor(input[i])
        else:
            in_dict = {self.input_name: cast_pytorch_tensor(input)}
        if isinstance(output, list):
            #assert len(self.executor.get_outputs()) == len(output)
            for i in range(len(output)):
                out_dict[self.executor.get_outputs()[i].name] = cast_pytorch_tensor(output[i])
        else:
