...
extern "C" __global__ void default_function_kernel0(float* __restrict__ input0, float* __restrict__ input1, float* __restrict__ output0) {
    ...
    for (int i = tid; i < 2048; i += blockDim.x)
        sum += input0[i] * input1[blockIdx.x * 2048 + i];
...
int main() {
    ...
    cudaMallocManaged(&input0, 2048 * sizeof(float));
    cudaMallocManaged(&input1, 20480 * sizeof(float));
    cudaMallocManaged(&output0, 10 * sizeof(float));
    for (int i = 0; i < 2048; i++) input0[i] = i;
    for (int i = 0; i < 20480; i++) input1[i] = i;
...
// answer
// s = list(range(0, 2048))
// print([sum([x * (2048 * i + x) for x in s]) for i in range(10)])
