...
class CodeGeneratorR:
  ...
  def calc_grid(self, reduce_iters, space_iters, vthread=True):
    ...
    self.blck_grid = [
        blck_dict['blockIdx.x'], blck_dict['blockIdx.y'],
        blck_dict['blockIdx.z']
    ]
    self.thrd_grid = [
        thrd_dict['threadIdx.x'], thrd_dict['threadIdx.y'],
  ...
  def rewrite_schedule(self,
    ...
    for out in output_tensors:
      ...
      for axis in self.sche[out].op.axis:
        ...
        if self.bank_size != 4:
          ...
          if self.tiling[axis.var.name][-3] >= (self.bank_size // 4):
            self.tiling[axis.var.name][-3] = self.tiling[axis.var.name][-3] // (
                self.bank_size // 4)
            self.tiling[axis.var.name][-1] = self.tiling[axis.var.name][-1] * (
                self.bank_size // 4)
          else:
            print('shared mem tiling is too small.')
            self.tiling[axis.var.name][-1] = self.tiling[
                axis.var.name][-1] * self.tiling[axis.var.name][-3]
      ...
      if self.binding['space'][0] is not None:
        self.sche[out].bind(blck_fused,
                            te.thread_axis(self.binding['space'][0]))
      ...
      if self.binding['space'][2] is not None:
        self.sche[out].bind(thrd_fused,
                            te.thread_axis(self.binding['space'][2]))
    ...
    for out in output_tensors:
      ...
      if self.need_smem_tiling:
        for input_tensor in input_tensors:
          ...
          # print(input_tensor)
          shared_tensor = self.sche.cache_read(input_tensor, 'shared',
                                               [reg_tile])
          smem_tensor.append(shared_tensor)

        for shared_tensor in smem_tensor:
          local_tensor = self.sche.cache_read(shared_tensor, 'local',
                                              [reg_tile])
          reg_tensor.append(local_tensor)
