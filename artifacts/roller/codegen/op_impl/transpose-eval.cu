...
int main(int argc, char *argv[])
{
        ...
        Bh = (float*)malloc(input_size0 * sizeof(float));
        ...
        for (int i = 0; i < output_size; ++ i) Ch[i] = 0;
        ...
        // }
        // checkCudaErrors(cuMemcpyDtoH(Ch, Cd, output_size * sizeof(float)));
        // for (int i = 0; i < M; i++)
        //         for (int j = 0; j < N; j++)
        //                 if (Ch[i * N + j] != Ah[j * M + i])
        //                         printf("mismatch C[%d, %d]: %f A[%d, %d]: %f\n",
        //                                 i, j, Ch[i * N + j], j, i, Ah[j * M + i]);
