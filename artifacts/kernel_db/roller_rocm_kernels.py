...
from roller.test_config.get_func import get_func
import sys
from tvm import te
import time
...
def compile_and_run_kernel(rprog, op, shape, data_type, arch, policy, device_id, idx, prefix, config):
    ...
    # for rocm
    os.system("hipcc {}.cpp -O2 -o {} && " \
        "export HIP_VISIBLE_DEVICES={} && " \
        "./{} > {} && " \
        "rm {} ".format(file_name, file_name, device_id, file_name, log_name, file_name))
...
def run(identifier, arch, fix_block_size=None, tag=""):
    ...
    threads = []
    prefix = f"tmp_roller/{func_name}_" + "_".join([str(x) for x in shape]) + tag
...
if __name__ == '__main__':
    ...
    with open("rocm_tune_list") as f:
        ids = f.readlines()
        identifier = ids[arguments.tid].strip()
        print(identifier)

    run(identifier, "MI100", tag=f"_t{arguments.tid}")