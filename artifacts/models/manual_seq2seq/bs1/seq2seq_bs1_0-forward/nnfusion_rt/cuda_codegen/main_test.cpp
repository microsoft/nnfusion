...
int main(int argc, char *argv[]){
    ...
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Parameter_13_0_host, sizeof(float)* 256));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Parameter_13_0, sizeof(float) * 256));
    //input argument
    ...
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Parameter_16_0_host, sizeof(int64_t)* 1));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Parameter_16_0, sizeof(int64_t) * 1));
    //input argument
    int64_t* Parameter_17_0_host, *Parameter_17_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Parameter_17_0_host, sizeof(int64_t)* 50));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Parameter_17_0, sizeof(int64_t) * 50));
    //input argument
    float* Parameter_18_0_host, *Parameter_18_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Parameter_18_0_host, sizeof(float)* 189850));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Parameter_18_0, sizeof(float) * 189850));

    //output arguments
    float* Result_96_0_host, *Result_96_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_96_0_host, sizeof(float) * 256));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Result_96_0, sizeof(float) * 256));
    ...
    float* Result_98_0_host, *Result_98_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_98_0_host, sizeof(float) * 256));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Result_98_0, sizeof(float) * 256));
    int64_t* Result_99_0_host, *Result_99_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_99_0_host, sizeof(int64_t) * 1));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Result_99_0, sizeof(int64_t) * 1));
    int64_t* Result_100_0_host, *Result_100_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_100_0_host, sizeof(int64_t) * 50));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Result_100_0, sizeof(int64_t) * 50));
    char* Result_101_0_host, *Result_101_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_101_0_host, sizeof(char) * 1));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Result_101_0, sizeof(char) * 1));

    // fill input values
    for (int i = 0; i < 256; ++i) Parameter_13_0_host[i] = 1.0f;
    for (int i = 0; i < 1; ++i) Parameter_14_0_host[i] = 1.0f;
    for (int i = 0; i < 256; ++i) Parameter_15_0_host[i] = 1.0f;
    for (int i = 0; i < 1; ++i) Parameter_16_0_host[i] = 1.0f;
    for (int i = 0; i < 50; ++i) Parameter_17_0_host[i] = 1.0f;
    for (int i = 0; i < 189850; ++i) Parameter_18_0_host[i] = 1.0f;


    //warm up for 5 iters:
    for(int i_=0; i_< 100; i_++)
    {
        CUDA_SAFE_CALL(cudaMemcpy(Parameter_13_0, Parameter_13_0_host, sizeof(float) * 256, cudaMemcpyHostToDevice));
        ...
        CUDA_SAFE_CALL(cudaMemcpy(Parameter_16_0, Parameter_16_0_host, sizeof(int64_t) * 1, cudaMemcpyHostToDevice));
        CUDA_SAFE_CALL(cudaMemcpy(Parameter_17_0, Parameter_17_0_host, sizeof(int64_t) * 50, cudaMemcpyHostToDevice));
        CUDA_SAFE_CALL(cudaMemcpy(Parameter_18_0, Parameter_18_0_host, sizeof(float) * 189850, cudaMemcpyHostToDevice));
        ...
        CUDA_SAFE_CALL(cudaMemcpy(Result_96_0_host, Result_96_0,  sizeof(float) * 256, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaMemcpy(Result_97_0_host, Result_97_0,  sizeof(int64_t) * 1, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaMemcpy(Result_98_0_host, Result_98_0,  sizeof(float) * 256, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaMemcpy(Result_99_0_host, Result_99_0,  sizeof(int64_t) * 1, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaMemcpy(Result_100_0_host, Result_100_0,  sizeof(int64_t) * 50, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaMemcpy(Result_101_0_host, Result_101_0,  sizeof(char) * 1, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        printf("%s \n", "Result_96_0:");
        for (int i = 0; i < 10; ++i) printf("%e ", (float)Result_96_0_host[i]); 
        printf(" .. (size = 256, ends with %e);\n", (float)Result_96_0_host[255]);
        printf("%s \n", "Result_97_0:");
