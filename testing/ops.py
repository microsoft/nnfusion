...
def conv_nchw_implicit_gemm(n, f, h, w, c, kh, kw, s, d, p):
    if p == 0:
        return conv_nchw_implicit_gemm_no_pad(n, f, h, w, c, kh, kw, s, d, p)
    inh = (h - 1) * s + (kh - 1) * d + 1 - 2 * p
    inw = (w - 1) * s + (kw - 1) * d + 1 - 2 * p
    padh = inh + 2 * p
    padw = inw + 2 * p
    ...
    ir = f"pad[N, C, H0, W0] = input0[N, C, H0-{p}, W0-{p}].when([H0>={p}, H0<{inh+p}, W0>={p}, W0<{inw+p}], const(0.0).cast(input0[N, C, H0-{p}, W0-{p}].dtype())) where H0 in {padh}, W0 in {padw}; \
           ...
           kernel[M, K] = input1[M, K//{kh*kw}, K%{kh*kw}//{kw}, K%{kw}] where K in {kh*kw*c}, M in {f}; \
...
def conv_nchw_implicit_gemm_no_pad(n, f, h, w, c, kh, kw, s, d, p):
    assert p == 0
    inh = (h - 1) * s + (kh - 1) * d + 1 - 2 * p
    inw = (w - 1) * s + (kw - 1) * d + 1 - 2 * p
