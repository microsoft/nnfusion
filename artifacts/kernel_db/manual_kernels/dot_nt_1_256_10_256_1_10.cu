...
extern "C" __global__ void default_function_kernel0(float* __restrict__ input0, float* __restrict__ input1, float* __restrict__ output0) {
    float sum = input0[threadIdx.x] * input1[blockIdx.x * blockDim.x + threadIdx.x];
    int tid = threadIdx.x;
    ...
    if (tid < 32) {
        ...
        if (tid == 0) output0[blockIdx.x] = s; // s / 56 / 56
    }
}
// %%%
...
int main() {
    ...
    cudaMallocManaged(&input1, 10 * 256 * sizeof(float));
    cudaMallocManaged(&output0, 10 * sizeof(float));
    for (int i = 0; i < 256; i++) input0[i] = i;
    for (int i = 0; i < 10 * 256; i++) input1[i] = i;
...
// answer
// s = list(range(0, 256))
// [sum([(256 * i + x) * x for x in s]) for i in range(10)]
// [5559680, 13915520, 22271360, 30627200, 38983040, 47338880, 55694720, 64050560, 72406400, 80762240]