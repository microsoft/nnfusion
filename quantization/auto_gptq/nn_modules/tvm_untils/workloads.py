...
if __name__ == '__main__':
    ...
    parser.add_argument("--N", type=int, default=17920)
    ...
    if args.M == 1:
        ...
        scales_np = np.random.rand(N * (K // group_size)).reshape((N, K // group_size)).astype("float16").T
        zeros_np = np.random.rand(N * (K // group_size)).reshape((N, K // group_size)).astype("float16").T
