...
extern "C" __global__ void __launch_bounds__(1024) default_function_kernel0(float* __restrict__ data, float* __restrict__ tensor) {
  ...
  tensor1[(0)] = (tensor1[(0)] + data[(((((((((int)blockIdx.x) * 1024) + ((int)threadIdx.x)) / 1764) * 6889) + (((((((int)blockIdx.x) * 1024) + ((int)threadIdx.x)) % 1764) / 42) * 166)) + ((((((int)blockIdx.x) * 1024) + ((int)threadIdx.x)) % 42) * 2)))]);
