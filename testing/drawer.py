    ...
    for op_id, op in enumerate(graph.node):
        ...
        for input_name in op.input:
            if input_name not in pydot_nodes:
                input_node = pydot.Node(
                    _escape_label(
                        input_name[input_name.rfind("/")+1:] + str(pydot_node_counts[input_name])),
                    label=_escape_label(input_name[input_name.rfind("/")+1:]),
                    **BLOB_STYLE
                )
        ...
        for output_name in op.output:
            ...
            output_node = pydot.Node(
                _escape_label(
                    output_name[output_name.rfind("/")+1:] + str(pydot_node_counts[output_name])),
                label=_escape_label(output_name[output_name.rfind("/")+1:]),
                **BLOB_STYLE
            )
...
def main() -> None:
    ...
    os.system("dot -Tpdf {} -o {}".format(args.output, args.output))
