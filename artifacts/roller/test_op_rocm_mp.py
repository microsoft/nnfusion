...
def compile_and_run_kernel(rprog, op, arch, policy, device_id, log_name, idx):
    ...
    # for rocm
    os.system("/opt/rocm/bin/hipcc {}.cpp -O2 -o {} && " \
        "export HIP_VISIBLE_DEVICES={} && " \
        "./{} > {} && " \
        "rm {} && " \
        "rm {}.cpp".format(file_name, file_name, device_id, file_name, log_name, file_name, file_name))

    return get_time_from_rocm_file(log_name)  # for rocm
...
def eval_thread(rprogs, rprog_idx, device_id, op, arch, policy):
    ...
    for idx in range(rprog_idx[device_id], rprog_idx[device_id + 1]):
        rprog = rprogs[idx]
        exec_time = compile_and_run_kernel(rprog, op, arch, policy, device_id, log_name, idx)
    ...
    return best_time, best_idx, top1_time
...
if __name__ == '__main__':
    ...
    for thread in threads:
        thread.join()
        local_best_time, local_best_idx, this_top1_time = thread.get_result()
