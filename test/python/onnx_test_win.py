...
class TestContext:
    ...
    nnfusion_port = 65432
    msbuild_bin = 'D:/Programs/"Microsoft Visual Studio"/2022/Enterprise/MSBuild/Current/Bin/MSBuild.exe'
    # remain unchanged
    ...
    nnfusion_device = "HLSL"
    nnfusion_codegen_dir = "nnfusion_rt/cuda_codegen"

    def __init__(self, keep_existing = False) -> None:
        ...
        if not keep_existing and os.path.exists(self.nnfusion_workdir):
            shutil.rmtree(self.nnfusion_workdir)
        os.makedirs(self.nnfusion_workdir, exist_ok = True)

    def test_pipeline(self, test_cases):
        case_count = 0
        for case in load_model_tests(data_dir=self.onnx_tests, kind=self.onnx_test_kind):
            if case.name in test_cases:
                opname = case.name.split('_')[1]
                case_count += 1
                print('Test [%d/%d]:'%(case_count, len(test_cases)), case.name, opname)
                if os.path.exists(os.path.join(self.nnfusion_workdir, case.name)):
                    print('Skipped.')
                    continue
    ...
    def _build_model(self, model_test):
        ...
        if isinstance(model_test, str):
            if not model_test.endswith(".onnx"):
                ...
                nnfusion_workdir = os.path.join(self.nnfusion_workdir, os.path.basename(model_test)[:-5])
        ...
        if not os.path.exists(nnfusion_workdir):
            os.makedirs(nnfusion_workdir, exist_ok = True)
        if platform.system().lower() == 'linux':
            ...
            # On windows use socket to communicate between host and WSL,
            # and the nnfusion_wsl_codegen_server.py must be started on WSL 
            # before runing this testing pipeline.
            params = {
                'model' : os.path.abspath(model_pb_path),
                'output_dir' : os.path.abspath(nnfusion_workdir),
            }
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.connect(('127.0.0.1', self.nnfusion_port))
                s.sendall(bytes(json.dumps(params), 'utf-8'))
                ret = s.recv(10240)
                ret = json.loads(ret.decode())
                if not ret['ret']:
                    raise RuntimeError(ret['error'])
            rt_dir = os.path.join(nnfusion_workdir, "nnfusion_rt/dxcompute_codegen/Direct3DWinNN")
            to_cst_dir = os.path.join(rt_dir, "nnf_desktop_example/Constant")
            cur_cst_dir = os.path.join(self.root_dir, "Constant")
            with cd(rt_dir):
                execute(f'{self.msbuild_bin} ./nnf_desktop_example.sln /p:PlatformToolset=v143 -p:Configuration=Release')
            return Executor(os.path.join(rt_dir, 'x64/Release'))
    ...
    def run_input_as_constant(self, model_test, op_name) -> None:
        ...
        for test_data_dir in glob.glob(os.path.join(model_dir, "test_data_set*")):
            ...
            try:
                ...
                new_pb_path = os.path.join(self.nnfusion_workdir, model_test.name + '.onnx')
            ...
            try:
                ...
                for output_i in range(len(ref_outputs)):
                    ...
                    nnf_torch_outputs.append(torch.tensor(ref_outputs[output_i]))
                    nnf_torch_outputs[-1].zero_()
                    if self.nnfusion_device == 'CUDA':
                        nnf_outputs[name] = cast_pytorch_tensor(nnf_torch_outputs[-1].to(torch_device))
                    elif self.nnfusion_device == 'HLSL':
                        nnf_outputs[name] = cast_hlsl_tensor(HLSLTensor.build_from_torch(nnf_torch_outputs[-1]))
            ...
            if not r:
                print('expected output:', ref_outputs)
        ...
        for test_data_dir in glob.glob(os.path.join(model_dir, "test_data_set*")):
            ...
            try:
                ...
                for input_i in range(len(inputs)):
                    ...
                    nnf_torch_inputs.append(torch.tensor(inputs[input_i]))
                    if self.nnfusion_device == 'CUDA':
                        nnf_inputs[name] = cast_pytorch_tensor(nnf_torch_inputs[-1].to(torch_device))
                    elif self.nnfusion_device == 'HLSL':
                        nnf_inputs[name] = cast_hlsl_tensor(HLSLTensor.build_from_torch(nnf_torch_inputs[-1]))
                nnf_outputs = dict()
                nnf_torch_outputs = list()
                for output_i in range(len(ref_outputs)):
                    name = model.graph.output[output_i].name
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--name', default="test_cos") 
    parser.add_argument('-f', '--file', default="user_pass_list.csv") 
    ...
    if args.mode == "name":
        test_cases = args.name.lower().split(",")
        test_cases = [v.strip() for v in test_cases]
    elif args.mode == "file":
        f = open(args.file).readlines()
        test_cases = [v.strip().lower().split(',')[0].strip() for v in f if v != '\n']
    test_context = TestContext(args.resume)
    test_context.test_pipeline(set(test_cases))
