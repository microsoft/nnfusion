...
extern "C" __global__ void __launch_bounds__(256) default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ conv_unpad) {
  ...
  for (int ra_fused0_outer = 0; ra_fused0_outer < 32; ++ra_fused0_outer) {
    __syncthreads();
    data_pad_shared[((int)threadIdx.x)] = (((((((int)blockIdx.x) & 3) * 16) + (((int)threadIdx.x) & 15)) < 49) ? data[((((ra_fused0_outer * 784) + ((((int)threadIdx.x) >> 4) * 49)) + ((((int)blockIdx.x) & 3) * 16)) + (((int)threadIdx.x) & 15))] : 0.000000e+00f);
    kernel_pad_shared[((int)threadIdx.x)] = kernel[(((((((int)blockIdx.x) >> 2) * 16384) + ((((int)threadIdx.x) >> 4) * 512)) + (ra_fused0_outer * 16)) + (((int)threadIdx.x) & 15))];
    kernel_pad_shared[(((int)threadIdx.x) + 256)] = kernel[((((((((int)blockIdx.x) >> 2) * 16384) + ((((int)threadIdx.x) >> 4) * 512)) + (ra_fused0_outer * 16)) + (((int)threadIdx.x) & 15)) + 8192)];
    __syncthreads();
