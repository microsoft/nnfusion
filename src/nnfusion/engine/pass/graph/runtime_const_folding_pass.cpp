...
RuntimeConstantFoldingPass::thread_pool::thread_pool() : stopped{ false }
{
    idlThrNum = std::thread::hardware_concurrency();
    for (int size = 0; size < idlThrNum; ++size)
    {
        pool.emplace_back(
            [this]
            {
...
void RuntimeConstantFoldingPass::runtime_const_folding_node(
    std::shared_ptr<Graph>& graph, std::set<std::shared_ptr<GNode>>& blocklist_nodes, std::shared_ptr<GNode>& node)
{
...
void RuntimeConstantFoldingPass::runtime_const_folding_task(
    std::shared_ptr<Graph>& graph, std::set<std::shared_ptr<GNode>>& blocklist_nodes, std::shared_ptr<GNode>& node, std::map<std::shared_ptr<GNode>, int>& in_degree, std::mutex& in_degree_lock)
{
    if (!node->is_constant())
    {
        runtime_const_folding_node(graph, blocklist_nodes, node);
    }
    ...
    for (auto& output : node->get_out_edges())
    {
        ...
        NNFUSION_CHECK(output->get_src() == node);
        {
            ...
            if (it != in_degree.end() && it->second > 0)
            {
                ...
                if (it->second == 0)
                {
                    pool.commit([this, &graph, &blocklist_nodes, &target, &in_degree, &in_degree_lock]() { this->runtime_const_folding_task(graph, blocklist_nodes, target, in_degree, in_degree_lock); });
                }
            }
            else
            {
    ...
    std::set<std::shared_ptr<GNode>>& blocklist_nodes)
{
    ...
    for (auto& it : nodes)
    {
        if (in_degree[it] == 0)
        {
            pool.commit([this, &graph, &blocklist_nodes, &it, &in_degree, &in_degree_lock]() { this->runtime_const_folding_task(graph, blocklist_nodes, it, in_degree, in_degree_lock); });
        }
    }
}
