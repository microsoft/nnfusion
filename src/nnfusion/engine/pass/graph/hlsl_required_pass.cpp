...
namespace
{
        ...
        for (auto out_edge : old_node->get_out_edges())
        {
            ...
            {                               
                auto new_input = make_shared<nnfusion::graph::Input>(
                    new_node->get_element_type(), new_node->get_shape());
    ...
    void revalidate_gnode(std::shared_ptr<GNode> node, std::shared_ptr<nnfusion::graph::Graph>& graph)
    {
        for (auto in_edge : node->get_in_edges())
        {
            ...
            if (cur_input->get_element_type() != src_output->get_element_type() || cur_input->get_shape() != src_output->get_shape()) 
            {
                auto new_input = make_shared<nnfusion::graph::Input>(
...
bool HLSLRequiredPass::run_on_graph(std::shared_ptr<nnfusion::graph::Graph>& graph)
{
    ...
    map<element::Type, element::Type> type_mapping = {
    ...
    for (auto node : gnodes)
    {
        if (node->is_parameter())
        {
            ...
            if (iter != type_mapping.end())
            {
                ...
                NNFUSION_LOG(INFO) << "Convert Parameter " << node->get_name() << ": " << src_type.c_type_string() << " -> " << dst_type.c_type_string();
        ...
        else if (node->is_constant())
        {
            ...
            if (iter != type_mapping.end())
            {
                auto src_type = iter->first;
                auto dst_type = iter->second;
                NNFUSION_LOG(INFO) << "Convert Constant " << node->get_name() << ": " << src_type.c_type_string() << " -> " << dst_type.c_type_string();
                auto old_op = std::dynamic_pointer_cast<op::Constant>(node->get_op_ptr());
                NNFUSION_CHECK(old_op);
                auto data = old_op->get_value_strings();

                auto new_op = std::make_shared<op::Constant>(
                    dst_type,
                    node->get_shape(),
                    data);
                new_op->set_name(node->get_name());
        ...
        else if (node->get_op_type() == "Convert")
        {
            ...
            if (iter != type_mapping.end())
            {
                auto src_type = iter->first;
                auto dst_type = iter->second;
                NNFUSION_LOG(INFO) << "Convert Convert " << node->get_name() << ": " << src_type.c_type_string() << " -> " << dst_type.c_type_string();
                auto old_op = std::dynamic_pointer_cast<op::Convert>(node->get_op_ptr());
                NNFUSION_CHECK(old_op);

                auto new_op = std::make_shared<op::Convert>(
                    dst_type);
                new_op->set_name(node->get_name());
