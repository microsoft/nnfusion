...
if not os.path.exists("/opt/rocm/bin/hcc"):
    logging.info("HCC is not existed, thus skip the test.")
    exit(0)
...
for pbfile in os.listdir(models):
    ...
    if not os.path.exists("nnfusion_rt/rocm_codegen/nnfusion_rt.cpp"):
        ...
        exit(2)
    os.system("cd nnfusion_rt/rocm_codegen/ && cmake . >> cmake.log && make -j 2>&1 >> cmake.log")
    ...
    if not os.path.exists("nnfusion_rt/rocm_codegen/result.txt"):
        logging.error("Failed at nvcc compiling phase.")
        exit(4)
    result_file = open("nnfusion_rt/rocm_codegen/result.txt")
