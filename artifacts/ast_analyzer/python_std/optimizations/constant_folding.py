...
from ast_analyzer.python_std.analyses import ConstantExpressions, ASTMatcher
from ast_analyzer.python_std.passmanager import Transformation
from ast_analyzer.python_std.tables import MODULES
from ast_analyzer.python_std.conversion import to_ast, ConversionError, ToNotEval, mangle
from ast_analyzer.python_std.analyses.ast_matcher import DamnTooLongPattern
from ast_analyzer.python_std.syntax import PythranSyntaxError
from ast_analyzer.python_std.utils import isintegral, isnum
...
class ConstantFolding(Transformation):
    ...
    def prepare(self, node):
        ...
        for module_name in MODULES:
            ...
            if module_name != '__dispatch__' and module_name not in self.env:
                alias_module_name = mangle(module_name)
                # zc: maybe I miss some passes?
        ...
        try:
            import astunparse
            dummy_module = ast.gast_to_ast(dummy_module)
            ast.fix_missing_locations(dummy_module)
            dummy_module = compile(dummy_module, '<constant_folding>', 'exec')
            eval(dummy_module, self.env)
        except AttributeError as e:
            # zc: error like "module 'torch' has no attribute 'view'"
            pass
    ...
    def generic_visit(self, node):
        if isinstance(node, ast.expr) and node in self.constant_expressions:
            ...
            fake_node = ast.gast_to_ast(fake_node)
            ...
            try:
                ...
                if "union" in e.args[0]:
                    ...
                    # these functions.
                    return Transformation.generic_visit(self, node)
                elif "torch" in e.args[0]:
                    # FIXME: zc - not use torch for tensor functions
                    return Transformation.generic_visit(self, node)
                raise
