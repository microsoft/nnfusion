...
extern "C" __global__ void __launch_bounds__(64) default_function_kernel0(float* __restrict__ data, float* __restrict__ avgpool2d) {
  ...
  padded_data_shared[((((int)threadIdx.x) + 64))] = data[((((((int)blockIdx.x) * 512) + ((int)threadIdx.x)) + 64))];
  padded_data_shared[((((int)threadIdx.x) + 128))] = data[((((((int)blockIdx.x) * 512) + ((int)threadIdx.x)) + 128))];
  padded_data_shared[((((int)threadIdx.x) + 192))] = data[((((((int)blockIdx.x) * 512) + ((int)threadIdx.x)) + 192))];
  padded_data_shared[((((int)threadIdx.x) + 256))] = data[((((((int)blockIdx.x) * 512) + ((int)threadIdx.x)) + 256))];
  padded_data_shared[((((int)threadIdx.x) + 320))] = data[((((((int)blockIdx.x) * 512) + ((int)threadIdx.x)) + 320))];
  padded_data_shared[((((int)threadIdx.x) + 384))] = data[((((((int)blockIdx.x) * 512) + ((int)threadIdx.x)) + 384))];
  padded_data_shared[((((int)threadIdx.x) + 448))] = data[((((((int)blockIdx.x) * 512) + ((int)threadIdx.x)) + 448))];
  __syncthreads();
  ...
  avgpool2d[((((((int)blockIdx.x) * 128) + ((((int)threadIdx.x) >> 3) * 16)) + (((int)threadIdx.x) & 7)))] = avgpool2d_local[(0)];
  avgpool2d[(((((((int)blockIdx.x) * 128) + ((((int)threadIdx.x) >> 3) * 16)) + (((int)threadIdx.x) & 7)) + 8))] = avgpool2d_local[(1)];
}
