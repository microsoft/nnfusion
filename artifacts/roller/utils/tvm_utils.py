...
__all__ = [
    'extract_shape_info',
    'build_tensors',
    'get_normalized_reduce_axis',
    'classify_tvm_op_tensors',
]
...
def extract_shape_info(op):
  ...
  def extractor(op):
    return [_.value for _ in op.output(0).shape
...
def build_tensors(op, shape):
  ...
  if isinstance(compute_expr, tvm.tir.Reduce):
    ...
    if compute_expr.op.name == 'tir.if_then_else':
      ...
      def extract_constraints(cur_expr):
        if isinstance(cur_expr, tvm.tir.expr.And):
          return extract_constraints(cur_expr.a) + extract_constraints(
              cur_expr.b)
    ...
    # scatter
    if compute_expr.op.name == 'tir.call_pure_extern' and compute_expr.args[
        0].value == '__builtin_set':
      load_exprs = extract_producer_load(
          compute_expr.args[1], is_recursive=True) + extract_producer_load(
              compute_expr.args[2])
      in_tensors = {}
      for load_expr in load_exprs:
        if '___' + load_expr.producer.name == op.output(0).name:
          ...
          producer_name, dims = load_expr.producer.name, calc_tensor_dim(
              load_expr)
          if producer_name in in_tensors:
            # trick here
    ...
    print('WARNING: type: {}, content: {}'.format(
        type(compute_expr), compute_expr))
    assert (False)
