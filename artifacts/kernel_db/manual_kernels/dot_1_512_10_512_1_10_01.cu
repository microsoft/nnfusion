...
extern "C" __global__ void default_function_kernel0(float* __restrict__ input0, float* __restrict__ input1, float* __restrict__ output0) {
    int tid = threadIdx.x;
    float sum = input0[tid] * input1[blockIdx.x * 512 + tid] + input0[tid + 256] * input1[blockIdx.x * 512 + tid + 256];
...
int main() {
    ...
    cudaMallocManaged(&input0, 512 * sizeof(float));
    cudaMallocManaged(&input1, 5120 * sizeof(float));
    cudaMallocManaged(&output0, 10 * sizeof(float));
    for (int i = 0; i < 512; i++) input0[i] = i;
    for (int i = 0; i < 5120; i++) input1[i] = i;
...
// answer
// s = list(range(0, 512))
// print([sum([x * (512 * i + x) for x in s]) for i in range(10)])
