...
void Constant_int64_t_cuda_Constant_28(cudaStream_t stream, int64_t* output0)
{
    ...
    cudaMemcpyAsync(output0, tmp_mem, 288, cudaMemcpyHostToDevice, stream);
    bin_file.close();
...
// Fused functions:
// Select_char_int64_t_int64_t_int64_t_cuda_Select_20<<<dim3(1, 1, 1), dim3(36, 1, 1), 0, 0>>>(Constant_19_0, Constant_18_0, Parameter_4_0, Select_20_0);
// Select_char_int64_t_int64_t_int64_t_cuda_Select_30<<<dim3(1, 1, 1), dim3(36, 1, 1), 0, 0>>>(Constant_29_0, Constant_28_0, Parameter_4_0, Select_30_0);
...
extern void Reshape_float_float_cuda_Reshape_22_Call(const dim3 &grids, const dim3 &blocks, unsigned mem, cudaStream_t stream, float* input0, float* output0) {
    Reshape_float_float_cuda_Reshape_22<<<grids, blocks, mem, stream>>>(input0, output0);
}
// Node name:	Result_53
// Description:	Result
// Input:
...
// Fused functions:
// ScatterND_float_int64_t_float_float_cuda_ScatterND_21<<<dim3(12, 1, 1), dim3(64, 1, 1), 0, 0>>>(Parameter_5_0, Select_20_0, Reshape_17_0, tensor_21);
// ScatterND_float_int64_t_float_float_cuda_ScatterND_31<<<dim3(12, 1, 1), dim3(64, 1, 1), 0, 0>>>(Parameter_7_0, Select_30_0, Reshape_27_0, tensor_31);
// Deduped function map: <src_function_name : deduped_function_name>
...
{
    if (input0 != output0)
        CUDA_SAFE_CALL(cudaMemcpyAsync(output0, input0,3072, cudaMemcpyDeviceToDevice, stream));
}
...
void Constant_char_cuda_Constant_19(cudaStream_t stream, char* output0)
{
    ...
    cudaMemcpyAsync(output0, tmp_mem, 36, cudaMemcpyHostToDevice, stream);
    bin_file.close();
...
//	- name: Broadcast_40_0	type: float	shape: Shape{1, 12, 1, 64}
// Fused functions:
// BatchMatMul_float_float_float_cuda_BatchMatMul_10<<<dim3(12, 1, 1), dim3(32, 1, 1), 0, 0>>>(Reshape_22_0, Reshape_9_0, BatchMatMul_10_0);
// BatchMatMul_float_float_float_cuda_BatchMatMul_14<<<dim3(12, 1, 1), dim3(32, 1, 1), 0, 0>>>(Reshape_22_0, Reshape_13_0, BatchMatMul_14_0);
// BatchMatMul_float_float_float_cuda_BatchMatMul_24<<<dim3(12, 1, 1), dim3(32, 1, 1), 0, 0>>>(Reshape_22_0, Reshape_23_0, BatchMatMul_24_0);
// Broadcast_float_float_cuda_Broadcast_40<<<dim3(12, 1, 1), dim3(64, 1, 1), 0, 0>>>(Reshape_39_0, Broadcast_40_0);
// Deduped function map: <src_function_name : deduped_function_name>
...
{
    if (thread_id >= 256){
        for (int i = 0; i < 3; i++) __syncthreads();
        return;
    }
    const dim3 blockDim(256, 1, 1);
    const dim3 gridDim(48, 1, 1);
    const dim3 blockIdx(block_id, 0, 0);
    float* s_C = (float*)(shared_buffer + 0);
    int batch_id = blockIdx.x / 4;
    int in_batch_id = blockIdx.x & 3;
    int wrap_id = threadIdx.x / 16;
    int in_wrap_id = threadIdx.x & 15;
...
extern "C" __global__  void BlockFusionKernel_float_float_float_float_float_float_float_float_float_cuda_BatchMatMul_BatchMatMul_BatchMatMul_Broadcast_1(float* input0, float* input1, float* input2, float* input3, float* input4, float* output0, float* output1, float* output2, float* output3)
{
    ...
    if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 47)
    {
    ...
    else if ((int)blockIdx.x >= 48 && (int)blockIdx.x <= 95)
    {
        BatchMatMul_float_float_float_cuda_BatchMatMul_10_block_kernel(input0, input2, output1, threadIdx.x, blockIdx.x - 48 + 0, shared_buffer);
    }
    else if ((int)blockIdx.x >= 96 && (int)blockIdx.x <= 143)
    {
        BatchMatMul_float_float_float_cuda_BatchMatMul_10_block_kernel(input0, input3, output2, threadIdx.x, blockIdx.x - 96 + 0, shared_buffer);
    }
    else if ((int)blockIdx.x >= 144 && (int)blockIdx.x <= 155)
    {
        Broadcast_float_float_cuda_Broadcast_40_block_kernel(input4, output3, threadIdx.x, blockIdx.x - 144 + 0, shared_buffer);
    }
...
CUDA_SAFE_CALL(cudaMalloc((void**)&group_0_CUDA_GPU0_allocator_memory_pool,28288));
CUDA_SAFE_CALL(cudaMemset((void*)group_0_CUDA_GPU0_allocator_memory_pool, 0, 28288));
...
CUDA_SAFE_CALL(cudaSetDevice(0));
CUDA_SAFE_CALL(cudaMalloc((void**)&group_persist_CUDA_GPU0_allocator_memory_pool,787392));
CUDA_SAFE_CALL(cudaMemset((void*)group_persist_CUDA_GPU0_allocator_memory_pool, 0, 787392));
...
BlockFusionKernel_float_float_float_float_float_float_float_float_float_cuda_BatchMatMul_BatchMatMul_BatchMatMul_Broadcast_1_Call(dim3(156, 1, 1), dim3(256, 1, 1), 0, 0, Reshape_22_0, Reshape_9_0, Reshape_13_0, Reshape_23_0, Reshape_39_0, BatchMatMul_10_0, BatchMatMul_14_0, BatchMatMul_24_0, Broadcast_40_0);
 // name=Reshape_25
...
// eliminated: Reshape_float_float_cuda_Reshape_22_Call(dim3(12, 1, 1), dim3(64, 1, 1), 0, 0, Reshape_46_0, Reshape_47_0);
 // name=x_2
BatchMatMul_float_float_float_cuda_BatchMatMul_49_Call(dim3(48, 1, 1), dim3(256, 1, 1), 0, 0, Reshape_47_0, Reshape_48_0, BatchMatMul_49_0);
 // name=Reshape_50
