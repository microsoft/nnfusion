...
def separable_conv2d_fixed_padding(inputs, filter_size, kernel_size, depth_multiplier, scope, stride):
  # print(filter_size, kernel_size, stride)
  # print(inputs.get_shape().as_list())

  if inputs.get_shape().as_list() == [64, 336, 42, 42] and kernel_size == 5 and stride == 2:
    inputs = tf.pad(inputs, [[0, 0], [0, 0], [1, 2], [1, 2]])
    ret = slim.separable_conv2d(
        inputs,
        filter_size,
        kernel_size,
        padding='VALID',
        ...
        scope=scope,
        stride=stride)
  elif inputs.get_shape().as_list() == [64, 336, 42, 42] and kernel_size == 7 and stride == 2:
    inputs = tf.pad(inputs, [[0, 0], [0, 0], [2, 3], [2, 3]])
    ret = slim.separable_conv2d(
        inputs,
        filter_size,
        kernel_size,
  ...
  # print(inputs.get_shape().as_list(), ret.get_shape().as_list())
...
def _stacked_separable_conv(net, stride, operation, filter_size):
  ...
  for layer_num in range(num_layers - 1):
    net = tf.nn.relu(net)
    net = separable_conv2d_fixed_padding(
        net,
        filter_size,
        kernel_size,
        depth_multiplier=1,
