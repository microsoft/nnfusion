...
def get_log_filename(N, CI, H, W, CO, KH, KW, strides, padding, path):
    ...
    if op2 is not None:
        suf = suf + "_" + op2
    return os.path.join(path, "ansor_conv2d_{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}_{8}{9}.log".format(N, CI, H, W, CO, KH, KW, strides, padding, suf))
...
def main():
    ...
    #     op
    #     )
    # res = "python3 ${{INJ}} $1/${{prefix}}{0}.json".format(filename)
    # print(res)

    print(N, CI, H, W, CO, KH, KW, strides, padding, path)
    # log_filename = get_log_filename(N, CI, H, W, CO, KH, KW, strides, padding, path)
    # from os.path import exists
    # file_exists = exists(log_filename)
    # print(log_filename, file_exists)
