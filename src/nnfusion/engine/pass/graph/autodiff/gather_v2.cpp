...
REGISTER_BACKWARD_TRANSLATOR(GatherV2).translator([](std::shared_ptr<GNode> forward_node,
    ...
    if (forward_node->get_in_edge(0)->get_src()->get_name().find("word_embeddings") !=
        std::string::npos)
    {
        nnfusion::op::OpConfig::any myConfig;
        myConfig["x_shape"] = x_shape;
        auto gather_grad_op = std::make_shared<nnfusion::op::GenericOp>(
            forward_node->get_name() + "_x_grad", "EmbeddingGrad", myConfig);
        auto x_grad = graph->add_node_and_edge(gather_grad_op, {indices_index, outputs_grad[0]});
