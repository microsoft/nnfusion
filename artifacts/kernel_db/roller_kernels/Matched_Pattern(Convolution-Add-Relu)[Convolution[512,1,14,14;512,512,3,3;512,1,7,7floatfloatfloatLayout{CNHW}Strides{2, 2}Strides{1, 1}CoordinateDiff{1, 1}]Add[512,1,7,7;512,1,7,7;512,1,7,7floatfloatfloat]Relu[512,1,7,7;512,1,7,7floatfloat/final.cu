...
extern "C" __global__ void __launch_bounds__(256) default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ bias, float* __restrict__ conv_unpad) {
  ...
  for (int ra_fused0_outer = 0; ra_fused0_outer < 144; ++ra_fused0_outer) {
    __syncthreads();
    data_pad_shared[(((int)threadIdx.x))] = (((((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) < 49) && (0 < (((((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) % 49) / 7) * 2) + ((((ra_fused0_outer * 32) + (((int)threadIdx.x) >> 3)) % 9) / 3)))) && (0 < ((((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) % 7) * 2) + (((ra_fused0_outer * 32) + (((int)threadIdx.x) >> 3)) % 3)))) ? data[((((((((((ra_fused0_outer * 32) + (((int)threadIdx.x) >> 3)) / 9) * 196) + (((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) / 7) * 28)) + (((((ra_fused0_outer * 32) + (((int)threadIdx.x) >> 3)) % 9) / 3) * 14)) + (((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) % 7) * 2)) + (((ra_fused0_outer * 32) + (((int)threadIdx.x) >> 3)) % 3)) - 15))] : 0.000000e+00f);
