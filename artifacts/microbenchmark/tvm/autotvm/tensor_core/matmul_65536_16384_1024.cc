...
extern "C" __global__ void __launch_bounds__(256) matmul_kernel0(float* __restrict__ placeholder, float* __restrict__ placeholder1, float* __restrict__ T_matmul_NN) {
  ...
  for (int ax0_inner = 0; ax0_inner < 4; ++ax0_inner) {
    for (int ax1_inner_inner = 0; ax1_inner_inner < 4; ++ax1_inner_inner) {
      if (((((int)threadIdx.x) * 4) + ax1_inner_inner) < 8) {
        placeholder_shared[(((((((int)threadIdx.y) * 32) + (ax0_inner * 8)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner))] = placeholder[((((((((int)blockIdx.x) * 1048576) + (((int)threadIdx.y) * 65536)) + (ax0_inner * 16384)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner))];
      }
    }
  }
  ...
  for (int k_outer_outer = 0; k_outer_outer < 2047; ++k_outer_outer) {
    __syncthreads();
    for (int ax0_inner1 = 0; ax0_inner1 < 4; ++ax0_inner1) {
      for (int ax1_inner_inner2 = 0; ax1_inner_inner2 < 4; ++ax1_inner_inner2) {
        if (((((int)threadIdx.x) * 4) + ax1_inner_inner2) < 8) {
          if ((((k_outer_outer * 8) + (((int)threadIdx.x) * 4)) + ax1_inner_inner2) < 16376) {
            placeholder_shared[((((((((k_outer_outer + 1) & 1) * 512) + (((int)threadIdx.y) * 32)) + (ax0_inner1 * 8)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner2))] = placeholder[((((((((((int)blockIdx.x) * 1048576) + (((int)threadIdx.y) * 65536)) + (ax0_inner1 * 16384)) + (k_outer_outer * 8)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner2) + 8))];
          }
        }
      }
    }
