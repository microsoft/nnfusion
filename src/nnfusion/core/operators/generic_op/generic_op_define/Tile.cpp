...
REGISTER_OP(Tile)
    .infershape([](std::shared_ptr<graph::GNode> gnode) -> void {
        ...
        auto repeat_op = gnode->get_in_edge(1)->get_src();
        std::vector<int64_t> multiples;
        bool status = nnfusion::frontend::GetValueFromNGraphOp<int64_t>(repeat_op, &multiples);
        NNFUSION_CHECK(status) << "Failed to load repeats.";
        // NNFUSION_CHECK(repeat_op->is_constant())
        //     << "We only accept the Tile input \"multiples\" as Constant.";
        // auto multiples = std::dynamic_pointer_cast<nnfusion::op::Constant>(repeat_op->get_op_ptr())
        //                      ->get_vector<int64_t>();
        ...
        NNFUSION_LOG(INFO) << output_shape_0 << multiples << input_shape_0;
