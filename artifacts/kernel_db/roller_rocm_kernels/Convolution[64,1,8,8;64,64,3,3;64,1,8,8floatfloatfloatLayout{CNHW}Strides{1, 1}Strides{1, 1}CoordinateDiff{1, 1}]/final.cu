...
#endif
extern "C" __global__ void __launch_bounds__(512) default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ conv_unpad) {
  float conv_local[2];
  ...
  conv_unpad[(((((int)blockIdx.x) * 1024) + ((((int)threadIdx.x) >> 5) * 64)) + (((int)threadIdx.x) & 31))] = conv_local[0];
  conv_unpad[((((((int)blockIdx.x) * 1024) + ((((int)threadIdx.x) >> 5) * 64)) + (((int)threadIdx.x) & 31)) + 32)] = conv_local[1];
}
