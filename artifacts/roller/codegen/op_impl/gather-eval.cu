...
int main(int argc, char *argv[])
{
        ...
        for (int i = 0; i < input_size1; ++ i)
                Bh[i] = rand() % (input_size0);
        printf("RAND_MAX is %d\n", RAND_MAX);

        checkCudaErrors(cuMemcpyHtoD(Ad, Ah, input_size0 * sizeof(float)));
        checkCudaErrors(cuMemcpyHtoD(Bd, Bh, input_size1 * sizeof(int)));
        ...
        dim3 grid(N / 32, M / 32, 1);
        dim3 block(32, 32, 1);
        for (int i = 0; i < 10; ++i)
        {
                default_function_kernel0<<<grid, block>>>((float *)Cd, (float *)Ad, (int *)Bd);
                cudaDeviceSynchronize();
        }
}