...
def bank_conflict_penalty(schedule, tiling):
  ...
  for tensor_name in sub_op.dims:
    ...
    for tid in range(0, len(addrs[tensor_name]), warp_size):
      ...
      flattened_warp_addrs = [
          sub_op.flatten_addr(addr, tensor_name)
          for addr in addrs[tensor_name][tid:tid_r]
      ]
      ret[tensor_name] += Access_To_Delay(
          Bank_Count(flattened_warp_addrs, bank_size))
