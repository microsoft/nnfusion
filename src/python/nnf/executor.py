...
def parse_nnf_params(param_file):
    ...
    weights = convert_nnf_info(nnf_params.get("weight", dict()), is_nnf_input=True)
    inputs = convert_nnf_info(nnf_params.get("input", dict()), is_nnf_input=True)
    outputs = convert_nnf_info(nnf_params.get("output", dict()), is_nnf_input=False)
...
class Executor(object):
    ...
    def __init__(self, nnf_rt_dir):
        ...
        for weight in weights.values():
            input_descs[weight["id"]] = IODescription(weight["name"], weight["shape"], weight["dtype"])
            input_index[weight["name"]] = weight["id"]
        for input in inputs.values():
            input_descs[input["id"]] = IODescription(input["name"], input["shape"], input["dtype"])
            input_index[input["name"]] = input["id"]
        for output in outputs.values():
            output_descs[output["id"]] = IODescription(output["name"], output["shape"], output["dtype"])
    ...
    def feed_data(self, inputs, outputs):
        ...
        for name, data_format in inputs.items():
            ...
            if data_format.shape != self.input_descs[index].shape or data_format.dtype != self.input_descs[index].dtype:
                raise Exception(f"Shape or type mismatch for NNFusion model input {name}, expect [{self.input_descs[index].shape}, {self.input_descs[index].dtype}], feed [{data_format.shape}, {data_format.dtype}]")
        ...
        for name, data_format in outputs.items():
            ...
            if data_format.shape != self.output_descs[index].shape or data_format.dtype != self.output_descs[index].dtype:
                raise Exception(f"Shape or type mismatch for NNFusion model output {name}, expect [{self.output_descs[index].shape}, {self.output_descs[index].dtype}], feed [{data_format.shape}, {data_format.dtype}]")
            signature[len(self.input_descs) + index] = data_format.pointer_type
