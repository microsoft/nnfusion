#include <cuda_runtime.h>
#include <stdexcept>
#include <sstream>

#define CUDA_SAFE_CALL(x)                                                                          \
    do                                                                                             \
    {                                                                                              \
        cudaError_t result = (x);                                                                  \
        if (result != cudaSuccess)                                                                 \
        {                                                                                          \
            const char* msg = cudaGetErrorString(result);                                          \
            std::stringstream safe_call_ss;                                                        \
            safe_call_ss << "\nerror: " #x " failed with error"                                    \
                         << "\nfile: " << __FILE__ << "\nline: " << __LINE__ << "\nmsg: " << msg;  \
            throw std::runtime_error(safe_call_ss.str());                                          \
        }                                                                                          \
    } while (0)

// %%%
extern "C" __global__ void default_function_kernel0(float* __restrict__ input0, float* __restrict__ input1, float* __restrict__ output0) {
    float s = threadIdx.x < 10 ?  input0[threadIdx.x] * input1[threadIdx.x] : 0;
    s += __shfl_xor_sync(0xffffffff, s, 16);
    s += __shfl_xor_sync(0xffffffff, s, 8);
    s += __shfl_xor_sync(0xffffffff, s, 4);
    s += __shfl_xor_sync(0xffffffff, s, 2);
    s += __shfl_xor_sync(0xffffffff, s, 1);
    if (threadIdx.x == 0) output0[0] = s;
}
// %%%

// +++
dim3 grid(1, 1, 1);
dim3 block(32, 1, 1);
// +++

int main() {
    float *input0, *input1, *output0;
    cudaMallocManaged(&input0, 10 * sizeof(float));
    cudaMallocManaged(&input1, 10 * sizeof(float));
    cudaMallocManaged(&output0, 1 * sizeof(float));
    for (int i = 0; i < 10; i++) input0[i] = i;
    for (int i = 0; i < 10; i++) input1[i] = i;
    default_function_kernel0<<<grid, block>>>(input0, input1, output0);
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
    for (int i = 0; i < 1; i++) printf("%f ", output0[i]);
    return 0;
}

// answer
// s = list(range(0, 10))
// print(sum([x * x for x in s]))
// 285