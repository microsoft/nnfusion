        ...
        if (fusion_level > 0)
        {
            ...
            for (auto group : *fuse_groups)
            {
                ...
                case 1:
                    ...
                    NNFUSION_CHECK(FLAGS_fdefault_device == "ROCm");
...
private:
    ...
    void MergeGroups(std::shared_ptr<std::vector<std::shared_ptr<FusionGroup>>> groups)
    {
        ...
        while (group_index < groups->size())
        {
            ...
            if (target->merge)
            {
                ...
                merged->nodes.insert(
                    merged->nodes.end(), current->nodes.begin(), current->nodes.end());
    ...
    int FuseGroupOnGraph(const std::shared_ptr<FusionGroup> group)
    {
        ...
        for (size_t index = 0; index < group->block_kernels.size(); index++)
        {
            ...
            if (FLAGS_fblockfusion_level == 2)
            {
                if (nodes_dep.size() > 0)
                    virtual_device.schedule_kernel_with_dependency(
                        std::dynamic_pointer_cast<BlockCudaEmitter>(kernel), nodes_dep);
                else
                    virtual_device.schedule_kernel(
                        std::dynamic_pointer_cast<BlockCudaEmitter>(kernel));
            }
            else
            {
                if (nodes_dep.size() > 0)
                    ...
                    virtual_device.schedule_kernel(
                        std::dynamic_pointer_cast<BlockCudaEmitter>(kernel), kernel_metric);
            }
            nodes_dep.clear();
        }
