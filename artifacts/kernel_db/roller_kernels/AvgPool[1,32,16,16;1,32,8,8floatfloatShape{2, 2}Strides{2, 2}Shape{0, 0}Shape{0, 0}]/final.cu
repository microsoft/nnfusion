...
extern "C" __global__ void __launch_bounds__(64) default_function_kernel0(float* __restrict__ data, float* __restrict__ avgpool2d) {
  ...
  avgpool2d[((((((int)blockIdx.x) * 128) + ((((int)threadIdx.x) >> 2) * 8)) + (((int)threadIdx.x) & 3)))] = avgpool2d_local[(0)];
  avgpool2d[(((((((int)blockIdx.x) * 128) + ((((int)threadIdx.x) >> 2) * 8)) + (((int)threadIdx.x) & 3)) + 4))] = avgpool2d_local[(1)];
}
