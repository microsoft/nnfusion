...
extern "C" __global__ void __launch_bounds__(128) default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ bias, float* __restrict__ conv_unpad) {
  ...
  for (int ra_fused0_outer = 0; ra_fused0_outer < 64; ++ra_fused0_outer) {
    __syncthreads();
    data_pad_shared[((int)threadIdx.x)] = (((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) < 49) ? data[((((ra_fused0_outer * 1568) + ((((int)threadIdx.x) >> 3) * 49)) + (((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) / 7) * 7)) + (((((int)threadIdx.x) & 7) + (((int)blockIdx.x) % 7)) % 7))] : 0.000000e+00f);
    data_pad_shared[(((int)threadIdx.x) + 128)] = (((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) < 49) ? data[(((((ra_fused0_outer * 1568) + ((((int)threadIdx.x) >> 3) * 49)) + (((((((int)blockIdx.x) % 7) * 8) + (((int)threadIdx.x) & 7)) / 7) * 7)) + (((((int)threadIdx.x) & 7) + (((int)blockIdx.x) % 7)) % 7)) + 784)] : 0.000000e+00f);
