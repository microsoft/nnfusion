...
extern "C" __global__ void __launch_bounds__(32) default_function_kernel0(float* __restrict__ data, float* __restrict__ avgpool2d) {
  ...
  avgpool2d[((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 3) * 16)) + (((int)threadIdx.x) & 7)))] = avgpool2d_local[(0)];
  avgpool2d[(((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 3) * 16)) + (((int)threadIdx.x) & 7)) + 8))] = avgpool2d_local[(1)];
}
