...
if __name__ == "__main__":
    ...
    for sub_dir, model_string in zip(*get_sub_dirs(prefix)):
        ...
        ret = subprocess.run(["python", "../run_ansor.py", "--prefix", sub_dir, "--skip"], capture_output=True)

        stdout = ret.stdout.decode()
        if "Execution time summary:" not in stdout:
            continue
        stdout = stdout[stdout.index("Execution time summary:"):]
        print(stdout)
