...
class K80(Arch):
  ...
  def __init__(self, para_opt=True):
    ...
    self.reg_cap = [
        32768, 96
    ]  # max reg per block(from hw: 65536), max reg per thread TODO 0 get device, 1 what?
    self.smem_cap = [
        49152
    ]  # device query, max shared memory per block TODO get device?
    ...
    self.glbmem_sm_partition = [
        26, 32
    ]  # TODO 32: The number of warps per sm when global memory reaches the peak throughput
