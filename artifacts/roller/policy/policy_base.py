...
class PolicyBase():
  ...
  def update_rtile_storage_padding(self, rprog, arch, mem_level, smem_tiling,
                                   reg_tiling, st_align):
    if not st_align:
      return
    if mem_level == 0:  #smem
      ...
      if len(expr_out) == 3:
        ...
        align_info = self.get_align_info_fuse(
            rprog,
            arch,
            smem_tiling,
            reg_tiling,
        ...
        out_tensor = out_tensors[0]
        align_info = self.get_align_info(
            rprog,
            arch,
            smem_tiling,
            reg_tiling,
    ...
    for out in output_tensors:
      ...
      if smem_bool:
        for input_tensor in input_tensors:
          shared_tensor = self.sche_simu.cache_read(input_tensor, 'shared',
                                                    [out])
          smem_tensor.append(shared_tensor)

      if reg_bool:
        for shared_tensor in smem_tensor:
          local_tensor = self.sche_simu.cache_read(shared_tensor, 'local',
                                                   [out])
      ...
      if binding['space'][0] is not None:
        self.sche_simu[out].bind(blck_fused,
                                 te.thread_axis(binding['space'][0]))
      ...
      if binding['space'][1] is not None:
        self.sche_simu[out].bind(thrd_fused,
                                 te.thread_axis(binding['space'][1]))
      ...
      if reg_tile is not None:
        for rt in reg_tensor:
          self.sche_simu[rt].compute_at(self.sche_simu[reg_tile],
                                        reduce_axis[-1])
        for st in smem_tensor:
          self.sche_simu[st].compute_at(self.sche_simu[reg_tile],
                                        reduce_axis[0])
