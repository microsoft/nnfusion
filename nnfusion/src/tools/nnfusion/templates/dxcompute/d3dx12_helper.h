...
struct D3DDevice
{
    ...
    void Init()
    {
        ...
        D3D12_COMMAND_QUEUE_DESC commandQueueDesc = D3D12CommandQueueDesc(D3D12_COMMAND_LIST_TYPE_COMPUTE);
        IFE(pDevice->CreateCommandQueue(&commandQueueDesc, IID_PPV_ARGS(&pCommandQueue)));

        // Create a command allocator
        IFE(pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COMPUTE, IID_PPV_ARGS(&pCommandAllocator)));
