...
bool DotAlgoSelectPass::run_on_graph(std::shared_ptr<Graph>& graph)
{
    ...
    for (auto node : graph->get_ordered_ops()) {
        if (node->get_op_type() == "Dot") {
            ...
            size_t k = op->get_transpose_A() ? A_shape[A_shape.size() - 2] : A_shape[A_shape.size() - 1];
            ...
            if (k % split_k_factor == 0 && num_output_elem / (64 * 64) < kSM && k/split_k_factor>=32) {
                ...
                config["tc_enabled"] = FLAGS_ftc_rewrite && node->get_output_element_type(0) == nnfusion::element::f16;
                auto dot_op = make_shared<op::GenericOp>(node->get_name() + ".dot", "DotSplitK", config);
