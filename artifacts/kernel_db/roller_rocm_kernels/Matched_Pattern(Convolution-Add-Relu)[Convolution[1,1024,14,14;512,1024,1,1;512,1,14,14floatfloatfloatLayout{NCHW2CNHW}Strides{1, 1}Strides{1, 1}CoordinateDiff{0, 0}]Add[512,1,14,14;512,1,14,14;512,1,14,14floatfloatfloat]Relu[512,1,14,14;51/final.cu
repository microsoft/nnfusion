...
extern "C" __global__ void __launch_bounds__(256) default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ bias, float* __restrict__ conv_unpad) {
  ...
  for (int ra_fused0_outer = 0; ra_fused0_outer < 32; ++ra_fused0_outer) {
    __syncthreads();
    data_pad_shared[((int)threadIdx.x)] = (((((((int)blockIdx.x) % 13) * 4) + ((((int)threadIdx.x) & 15) >> 2)) < 49) ? data[((((ra_fused0_outer * 6272) + ((((int)threadIdx.x) >> 4) * 196)) + (((((((int)blockIdx.x) % 13) * 8) + ((((int)threadIdx.x) & 15) >> 1)) / 7) * 14)) + ((((((int)blockIdx.x) % 13) * 2) + (((int)threadIdx.x) & 15)) % 14))] : 0.000000e+00f);
    data_pad_shared[(((int)threadIdx.x) + 256)] = (((((((int)blockIdx.x) % 13) * 4) + ((((int)threadIdx.x) & 15) >> 2)) < 49) ? data[(((((ra_fused0_outer * 6272) + ((((int)threadIdx.x) >> 4) * 196)) + (((((((int)blockIdx.x) % 13) * 8) + ((((int)threadIdx.x) & 15) >> 1)) / 7) * 14)) + ((((((int)blockIdx.x) % 13) * 2) + (((int)threadIdx.x) & 15)) % 14)) + 3136)] : 0.000000e+00f);
