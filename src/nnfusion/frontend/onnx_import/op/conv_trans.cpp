        ...
        namespace onnx_import
        {
            namespace set_1
            {
                                                         ...
                                                         std::shared_ptr<nnfusion::graph::Graph> m_graph)
                {
                    ...
                    auto conv_trans_attrs = extract_conv_trans_attrs(node, x_gnode.get_shape(), 
                                                                     w_gnode.get_shape());
                    ...
                    op_config["kernel_shape"] = Shape(conv_attrs["kernel_shape"].begin(), conv_attrs["kernel_shape"].end());
                    op_config["strides"] = Strides(conv_attrs["strides"].begin(), conv_attrs["strides"].end());
                    op_config["dilations"] = Strides(conv_attrs["dilations"].begin(), conv_attrs["dilations"].end());
                    op_config["padding_above"] = CoordinateDiff(
                        conv_attrs["pads"].begin(), conv_attrs["pads"].begin() + conv_attrs["pads"].size() / 2);
                    op_config["padding_below"] = CoordinateDiff(
                        conv_attrs["pads"].begin() + conv_attrs["pads"].size() / 2, conv_attrs["pads"].end());

                    auto node_name = node_proto.output(0);
                    auto conv_trans_op = std::make_shared<op::GenericOp>(node_name + ".conv", "ConvTranspose", op_config);
                    auto conv_trans_gnode = m_graph->add_node_and_edge(conv_trans_op, {x_gnode, w_gnode});

                    if (input_indices.size() == 3)
                        conv_trans_gnode = attach_bias_gnode(input_indices[2], conv_trans_gnode, m_graph);
