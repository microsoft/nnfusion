...
def sche_gemm(sch: tvm.tir.Schedule):
    ...
    # layoutA = RowMajorLayout()
    # layoutB = RowMajorLayout()

    for idx in [0, 1]:
        SS = sch.cache_read(C, idx, "shared")
        sch.compute_at(SS, K_outer)
        # sch.transform_layout(C, ("read", idx), layoutA if idx==0 else layoutB)
        fused = sch.fuse(*sch.get_loops(SS)[-2:])
        vectorize_size = 4 if idx == 0 else 8
        ...
        if layout.requires_padding():
            sch.storage_align(SS, 0, axis=-2, factor=32, offset=vectorize_size)

    register_cutlass_warp_mma(warp_size_M, warp_size_N, chunk_size,
    ...
    oo, vec = sch.split(sch.get_loops(C_warp)[-1], factors=[None, layoutC.get_vectorize_size()])
    sch.vectorize(vec)
    sch.unroll(oo)
    sch.tensorize(sch.get_loops(block_init_c)[-2],
        register_mma_fill_intrin(warp_size_M, warp_size_N, "float16", layoutC.get())
    )
    sch.tensorize(sch.get_loops(C)[-3],
        register_gemm_intrin(
            warp_size_M, warp_size_N, chunk_size, "float16", "float16", False, False,
            layoutA, layoutB, layoutC, block_size_M // warp_size_M, block_size_N // warp_size_N)
    )
