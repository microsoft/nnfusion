...
extern "C" __global__ void __launch_bounds__(256) matmul_kernel0(float* __restrict__ placeholder, float* __restrict__ placeholder1, float* __restrict__ T_matmul_NN) {
  ...
  for (int ax1_inner_inner1 = 0; ax1_inner_inner1 < 4; ++ax1_inner_inner1) {
    if (((int)threadIdx.y) < 8) {
      placeholder_d_shared[((((((int)threadIdx.y) * 64) + (((int)threadIdx.x) * 4)) + ax1_inner_inner1))] = placeholder1[(((((((int)threadIdx.y) * 4096) + (((int)blockIdx.y) * 64)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner1))];
    }
  }
  for (int k_outer_outer = 0; k_outer_outer < 127; ++k_outer_outer) {
    ...
    for (int ax1_inner_inner3 = 0; ax1_inner_inner3 < 4; ++ax1_inner_inner3) {
      if (((int)threadIdx.y) < 8) {
        placeholder_d_shared[(((((((k_outer_outer + 1) & 1) * 512) + (((int)threadIdx.y) * 64)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner3))] = placeholder1[(((((((k_outer_outer * 32768) + (((int)threadIdx.y) * 4096)) + (((int)blockIdx.y) * 64)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner3) + 32768))];
      }
    }
  ...
  for (int j_inner_inner_inner = 0; j_inner_inner_inner < 2; ++j_inner_inner_inner) {
    for (int i_inner_inner_inner = 0; i_inner_inner_inner < 2; ++i_inner_inner_inner) {
      T_matmul_NN[(((((((((int)blockIdx.x) * 262144) + (((int)threadIdx.x) * 8192)) + (i_inner_inner_inner * 4096)) + (((int)blockIdx.y) * 64)) + (((int)threadIdx.y) * 2)) + j_inner_inner_inner))] = T_matmul_NN_local[(((i_inner_inner_inner * 2) + j_inner_inner_inner))];
      T_matmul_NN[((((((((((int)blockIdx.x) * 262144) + (((int)threadIdx.x) * 8192)) + (i_inner_inner_inner * 4096)) + (((int)blockIdx.y) * 64)) + (((int)threadIdx.y) * 2)) + j_inner_inner_inner) + 32))] = T_matmul_NN_local[((((i_inner_inner_inner * 2) + j_inner_inner_inner) + 8))];
      T_matmul_NN[((((((((((int)blockIdx.x) * 262144) + (((int)threadIdx.x) * 8192)) + (i_inner_inner_inner * 4096)) + (((int)blockIdx.y) * 64)) + (((int)threadIdx.y) * 2)) + j_inner_inner_inner) + 131072))] = T_matmul_NN_local[((((i_inner_inner_inner * 2) + j_inner_inner_inner) + 4))];
      T_matmul_NN[((((((((((int)blockIdx.x) * 262144) + (((int)threadIdx.x) * 8192)) + (i_inner_inner_inner * 4096)) + (((int)blockIdx.y) * 64)) + (((int)threadIdx.y) * 2)) + j_inner_inner_inner) + 131104))] = T_matmul_NN_local[((((i_inner_inner_inner * 2) + j_inner_inner_inner) + 12))];
    }
  }
}
