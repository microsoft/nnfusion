...
REGISTER_OP(ConvTranspose)
    ...
    .infershape([](std::shared_ptr<graph::GNode> gnode) -> void {
        ...
        NNFUSION_CHECK(kernel_shape[0] == kernel_shape[1]) << "ConvTranspose only sopport equal kernel size!";
        ...
        {
            output_shape[i + (is_nchw ? 2 : 1)] = (input_shape[i + (is_nchw ? 2 : 1)] - 1) * strides[i] + ((kernel_shape[i] - 1) * dilations[i] + 1) - padding_above[i] - padding_below[i];
        }
    ...
    .translate_v2([](std::shared_ptr<graph::GNode> gnode) -> std::string {
        auto expr_tmpl = R"( mediate0[C, F, KW, KH] = @input1@[C, F, KH, KW]; @output0@[N, F, WO, HO] +=! @input0@[N, C, (-@pad_h@ + HO + KH) // @stride_h@, (-@pad_w@ + WO + KW) // @stride_w@].when([(-@pad_h@ + HO + KH) // @stride_h@ >= 0, (-@pad_h@ + HO + KH) // @stride_h@ < @in_height@, (-@pad_w@ + WO + KW) // @stride_w@ >= 0, (-@pad_w@ + WO + KW) // @stride_w@ < @in_width@, (-@pad_h@ + HO + KH) % @stride_h@ == 0, (-@pad_w@ + WO + KW) % @stride_w@ == 0], const(0.0).cast(@input0@[0, 0, 0, 0].dtype())) * mediate0[C, F, KH, KW] where HO in @out_height@, WO in @out_width@; ## @: plan/convfwd_nchw_v1 )";
