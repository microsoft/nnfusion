...
REGISTER_OP(Resize)
    ...
    .translate_v2([](std::shared_ptr<graph::GNode> gnode) -> std::string {
        ...
        else if (mode == "LINEAR" && generic_op->localOpConfig.getRoot()["coordinate_transformation_mode"] != "align_corners")
        {
        ...
        else if (mode == "LINEAR" && generic_op->localOpConfig.getRoot()["coordinate_transformation_mode"] == "align_corners")
        {
            ...
            auto expression_template =
                ...
                "h_weight[CH] = h_map[CH] - h_map[CH].call('floor') where CH in @oh_shape@;"
                "w_map[CH] = CH * @w_scale@ where CH in @ow_shape@;"
                "w_weight[CH] = w_map[CH] - w_map[CH].call('floor') where CH in @ow_shape@;"
                "h_map_int[CH] = h_map[CH].cast('int') where CH in @oh_shape@;"
                "w_map_int[CH] = w_map[CH].cast('int') where CH in @ow_shape@;"
                "@output0@@output0_layout@ = @input0@@input00_layout@ * h_weight@h_layout@ * "
                "w_weight@w_layout@"
                "+ @input0@@input10_layout@ * (1.0 - h_weight@h_layout@) * w_weight@w_layout@"
                "+ @input0@@input01_layout@ * h_weight@h_layout@ * (1.0 - w_weight@w_layout@)"
