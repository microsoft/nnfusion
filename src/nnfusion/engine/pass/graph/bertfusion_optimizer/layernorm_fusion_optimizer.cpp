...
// bool LayerNormFusionOptimizer::Optimize()
// {
...
/*
bool LayerNormFusionOptimizer::FuseSubGraph(std::shared_ptr<GNode> starting_node,
                                            std::shared_ptr<GNode> ending_node)
{
    // find reducemean1
    std::vector<std::vector<std::shared_ptr<GNode>>> reducemean1_mainpath;
    ...
    // find path to reducemean2
    std::vector<std::string> pattern_to_reducemean2 = {
        reducemean1->get_op_type(), "Broadcast", "Subtract", "Power"};
    std::vector<std::vector<std::shared_ptr<GNode>>> all_paths_to_reducemean2;
    if (!FindPath(reducemean1, pattern_to_reducemean2, all_paths_to_reducemean2, false) ||
        all_paths_to_reducemean2.size() != 1)
    {
    ...
    auto subtract = all_paths_to_reducemean2[0][2];
    if (subtract != ending_node)
    {
        NNFUSION_LOG(NNFUSION_WARNING) << "Failed to find path to the second reducemean subgraph";
        return false;
    ...
    auto power_const = power_broadcast->get_in_edge(0)->get_src();
    nodes_to_remove.insert(all_paths_to_reducemean2[0].begin(), all_paths_to_reducemean2[0].end());
    nodes_to_remove.insert({power, power_broadcast, power_const});
    ...
    std::shared_ptr<GNode> epsilon, scale, bias, epsilon_broadcast, scale_broadcast, bias_broadcast;
    for (auto in_edge : add2->get_in_edges())
    {
        auto src = in_edge->get_src();
        if (src != reducemean2 && src->get_op_type() == "Broadcast")
        {
            epsilon_broadcast = src;
            epsilon = src->get_in_edge(0)->get_src();
        }
    }
    for (auto in_edge : multiply->get_in_edges())
    {
    ...
    nodes_to_remove.insert(all_paths_to_last_add[0].begin(), all_paths_to_last_add[0].end());
    nodes_to_remove.insert({epsilon_broadcast, scale_broadcast, bias_broadcast});
    ...
    auto layernorm_op = std::make_shared<nnfusion::op::GenericOp>(
        starting_node->get_name() + "_layernorm", "LayerNorm", myConfig);
    auto layernorm_gnode = m_graph->add_node_and_edge(
        layernorm_op, {GNodeIndex{starting_node, 0}, GNodeIndex{scale, 0}, GNodeIndex{bias, 0}});

    // replace edge
    std::set<std::shared_ptr<nnfusion::graph::Edge>> out_edges = last_add->get_out_edges();
    for (auto out_edge : out_edges)
    {
        auto dst = out_edge->get_dst();
        int y = out_edge->get_dst_input();
