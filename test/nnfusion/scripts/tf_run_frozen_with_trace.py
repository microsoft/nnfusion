...
for n in range(num_nodes):
    for m in range(len(ops[n].outputs)):
    #if len(ops[n].outputs) > 0:
        last_outputs.append(ops[n].outputs[m])
        last_outputs_inputs[ops[n].outputs[m].name] = [v.name for v in ops[n].inputs]
...
with tf.Session(graph=graph) as sess:
    ...
    for i in range(len(outputs)):
        ...
        if (len(out_flat) > 0):
            ...
            inputstr = ""
            if last_outputs[i].name in last_outputs_inputs.keys():
                inputstr = ", ".join(last_outputs_inputs[last_outputs[i].name])
            print(last_outputs[i].name + " <- " + inputstr)
