...
if __name__ == '__main__':
    ...
    for rprog in rprogs:
        ...
        os.system('/usr/local/cuda-10.2/bin/nvcc {}.cu -lcuda -gencode=arch={},code={} -o {}'.format(
            file_name,
            arch.compute_capability,
            arch.compute_capability,
            file_name)
        )
        os.system('/usr/local/cuda-10.2/bin/nvprof ./{} 2>&1 |tee {}'.format(file_name, tmp_file))
        os.system("rm {}".format(file_name))
        os.system("rm {}.cu".format(file_name))

        this_time = get_time_from_nvprof_file(tmp_file)
