...
for SRC_FILE in $(find . ${filter} -type f -not -name \
    '*apply_liscense.sh' -not -name '*check_liscense.sh' -and \( -name '*.cpp' -or -name '*.h*' -or -name '*.cu' -or -name '*.in' \) ); do
...
for SRC_FILE in $(find . ${filter} -type f -not -name \
    '*apply_liscense.sh' -not -name '*check_liscense.sh' -and \( -name 'CMakeLists.txt' -or -name '*.cmake' \
...
for SRC_FILE in $(find . ${filter} -type f -not -name \
    '*apply_liscense.sh' -not -name '*check_liscense.sh' -name '*.sh' ); do
    sed -i '/Copyright (c) Microsoft Corporation. All rights reserved./d' ${SRC_FILE} &&
    sed -i '/Licensed under the MIT License. See License.txt in the project root for license information./d' ${SRC_FILE} &&
    sed -i '/\#\s*Microsoft\s*(c)/Id' ${SRC_FILE} 
    if !(grep -q "Copyright" "${SRC_FILE}"); then
