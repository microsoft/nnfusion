...
def load_model(model_path: str):
    ...
    codegen_flags = {'autodiff': False, 'training_mode': False, 'extern_result_memory': True, 'codegen_unexist_kernel': True, 'product_name': 'V100', 'max_grid_dim': 80, 'max_block_dim': 256}
...
class GenModel(torch.autograd.Function):
    forward_executor = load_model("seq2seq_bs64_0-forward.onnx")
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.__seq2seq_bs64_0 = GenModel()

    def forward(self, encoder_output, std, h, c):
        batch_size = 64
        output_all = (torch.zeros(50, 64, dtype=torch.int64, device='cuda') + 0)
        output = torch.full((64,), 1, dtype=torch.int64, device='cuda')
        cond = True
        id = torch.zeros((), dtype=torch.int64, device='cuda')
        while cond:
            (h, c, _, cond, id, output) = self.__seq2seq_bs64_0.apply(h, c, std, output_all, id, output)
        return (output_all, h)
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        for i in range(0, len_dataset, batch_size):
            ...
            if i == 0: print(tokens[i:i+batch_size])
