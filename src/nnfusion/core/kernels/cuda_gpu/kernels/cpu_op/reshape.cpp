...
LanguageUnit_p cuda_cpu::Reshape::emit_dependency()
{
...
cuda_cpu::Reshape2D::Reshape2D(shared_ptr<KernelContext> ctx)
    : Reshape(ctx)
{
...
LanguageUnit_p cuda_cpu::Reshape2D::emit_function_body()
{
    ...
    auto& lu = *_lu;
    lu << "copy from cuda emitter, needs further change\n";
...
cuda_cpu::Reshape3D::Reshape3D(shared_ptr<KernelContext> ctx)
    : Reshape(ctx)
{
...
LanguageUnit_p cuda_cpu::Reshape3D::emit_function_body()
{
...
cuda_cpu::ReshapehD::ReshapehD(shared_ptr<KernelContext> ctx)
    : Reshape(ctx)
{
    block_size_x = 256;
    input_strides = nnfusion::row_major_strides(arg_shape);
...
LanguageUnit_p cuda_cpu::ReshapehD::emit_function_body()
{
...
cuda_cpu::ReshapeMemcpy::ReshapeMemcpy(shared_ptr<KernelContext> ctx)
    : CPUOpEmitter(ctx)
{
    NNFUSION_CHECK(ctx->outputs[0]->size(false) > 0) << "Invalid output shape for Reshape.";
    reshape = static_pointer_cast<nnfusion::op::Reshape>(ctx->gnode->get_op_ptr());
...
bool cuda_cpu::ReshapeMemcpy::is_eliminative()
{
...
LanguageUnit_p cuda_cpu::ReshapeMemcpy::emit_function_body()
{
...
LanguageUnit_p cuda_cpu::ReshapeMemcpy::emit_dependency()
{
    LanguageUnit_p _lu(new LanguageUnit(get_function_name() + "_dep"));
    _lu->require(header::cuda);
    return _lu;
...
LanguageUnit_p cuda_cpu::ReshapeMemcpy::emit_function_signature()
{
...
REGISTER_KERNEL_EMITTER(
    "Reshape",                                                                       // op_name
    Device(SINGLE_CPU).TypeConstraint(element::f32).Tag("cuda_kernel_2D").Priority(2), // attrs
    cuda_cpu::Reshape2D)                                                                 // constructor

REGISTER_KERNEL_EMITTER(
    "Reshape",                                                                       // op_name
    Device(SINGLE_CPU).TypeConstraint(element::f32).Tag("cuda_kernel_3D").Priority(2), // attrs
    cuda_cpu::Reshape3D)                                                                 // constructor

REGISTER_KERNEL_EMITTER(
    "Reshape",                                                                      // op_name
    Device(SINGLE_CPU).TypeConstraint(element::f32).Tag("cuda_kernel_D").Priority(3), // attrs
    cuda_cpu::ReshapehD)                                                                // constructor

REGISTER_KERNEL_EMITTER(
    "Reshape",                                                                 // op_name
    Device(SINGLE_CPU).TypeConstraint(element::f32).Tag("cuda_lib").Priority(2), // attrs
    cuda_cpu::ReshapeMemcpy)                                                       // constructor
