...
def topi_conv2d_nchw_codegen(input_shape, filter_shape, output_shape, strides, paddings, dilations, add_bias=True, add_relu=True):
    ...
    if add_bias:
        func = tvm.build(s, [A, W, bias, C], device, name="topi_fused_convolution_add_relu_op_float_i%d_%d_%d_%d_w%d_%d_%d_%d_o%d_%d_%d_%d_ws%d_%d_wd%d_%d_p%d_%d" % (
        ...
        func = tvm.build(s, [A, W, C], device, name="topi_fused_convolution_add_relu_op_float_i%d_%d_%d_%d_w%d_%d_%d_%d_o%d_%d_%d_%d_ws%d_%d_wd%d_%d_p%d_%d" % (
            input_n, input_c, input_h, input_w, filter_c, filter_input_c, filter_h, filter_w, output_n, output_c, output_h, output_w, stride_h, stride_w, dilation_h, dilation_w, padding_h, padding_w))
        func(a, w, c)
...
def get_tvm_topi_func_name(input_shape, filter_shape, output_shape, strides, paddings, dilations):
    ...
    padding_h, padding_w = paddings
    dilation_h, dilation_w = dilations
    func_name = "topi_fused_convolution_add_relu_op_float_i%d_%d_%d_%d_w%d_%d_%d_%d_o%d_%d_%d_%d_ws%d_%d_wd%d_%d_p%d_%d_kernel0" % (
        input_n, input_c, input_h, input_w, filter_c, filter_input_c, filter_h, filter_w, output_n, output_c, output_h, output_w, stride_h, stride_w, dilation_h, dilation_w, padding_h, padding_w)
    return func_name
