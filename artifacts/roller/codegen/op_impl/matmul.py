...
C = te.compute((M, N), lambda y, x: te.sum(A[y, k] * B[k, x], axis=k))
s = te.create_schedule(C.op)

generator = CodeGenerator()
tile_dict = {'k': [32], 'y': [16, 4], 'x': [16, 4]}
...
# func = tvm.build(s, [A, B, C], "cuda")
with open('matmul.cuh', 'w') as ouf:
    ouf.write('#ifndef KERNELH\n#define KERNELH\n')
    ouf.write(func.imported_modules[0].get_source())
    ouf.write('#endif\n')

ctx = tvm.gpu(0)
A_h = np.random.uniform(size=(M, K)).astype(A.dtype)
B_h = np.random.uniform(size=(K, N)).astype(B.dtype)
C_h = A_h.dot(B_h)
