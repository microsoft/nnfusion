//509952_1_1_83_1_1
//128_96_165_165_5_2_SAME
//dim3 grid(509952, 1, 1);
//dim3 block(83, 1, 1);
...
extern "C" __global__ void __launch_bounds__(83) default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ DepthwiseConv2d) {
  ...
  for (int di_outer_outer = 0; di_outer_outer < 5; ++di_outer_outer) {
    ...
    if (((int)threadIdx.x) < 10) {
      kernel_shared[(((int)threadIdx.x))] = kernel[(((((((((int)blockIdx.x) % 3984) / 83) * 50) + ((((int)threadIdx.x) / 5) * 25)) + (di_outer_outer * 5)) + (((int)threadIdx.x) % 5)))];
    }
    __syncthreads();
  ...
  for (int c_inner = 0; c_inner < 2; ++c_inner) {
    DepthwiseConv2d[((((((((int)blockIdx.x) / 83) * 13778) + (c_inner * 6889)) + ((((int)blockIdx.x) % 83) * 83)) + ((int)threadIdx.x)))] = DepthwiseConv2d_local[(c_inner)];
  }
}
