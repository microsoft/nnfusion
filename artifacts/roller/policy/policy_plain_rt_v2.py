...
class PolicyPlainRTV2(PolicyBase):
    """
        Constructing tiling schedules using DFS for sizes
        expand tiling sizes for alignment requirement
    """
    def __init__(self, op, arch, smem_tiling=False, reg_tiling=False, st_align=False,padding_threshold_cap=1.0, shrink_tiny=False,tile_tensor="output"):
        self.op = op
        self.arch = arch
    ...
    def try_shrink(self, rprog):
        ...
        while True:
            ...
            r_scores = self.DataReuseScore(rprog, aligned_sizes, 0)
    ...
    def emit_raw_configs(self, padding_threshold=0):
        ...
        uniTile = rTile(self.op.tvm_op, [1 for _ in range(len(self.op.Dimensions()))], self.op.SAxis(), self.op.RAxis(), self.op.GetTvmOutTensor())
