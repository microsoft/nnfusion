...
static const std::unordered_map<std::string, element_op> ElementOpMap = {
    ...
    {"Sigmoid", element_op("sigmoid", "const(1).cast(x0.dtype()) / (const(1).cast(x0.dtype()) + (-x0).call(`exp`))")},
    {"SigmoidBackprop",
     element_op("sigmoid_backprop", "x1 / (const(2).cast(x0.dtype()) + (-x0).call(`exp`) + (x0).call(`exp`))")},
