...
extern "C" __global__ void __launch_bounds__(224) conv_kernel0(float* __restrict__ placeholder, float* __restrict__ placeholder1, float* __restrict__ compute) {
    ...
    for (int rc_inner = 0; rc_inner < 8; ++rc_inner) {
      ...
      for (int ff_c = 0; ff_c < 2; ++ff_c) {
        ...
        for (int yy_c = 0; yy_c < 7; ++yy_c) {
          ...
          compute_local[((((ff_c * 7) + yy_c) + 14))] = (compute_local[((((ff_c * 7) + yy_c) + 14))] + (pad_temp_shared[((((rc_inner * 169) + (yy_c * 26)) + (((int)threadIdx.x) * 2)))] * placeholder_shared[(((((((int)threadIdx.z) * 16) + (ff_c * 8)) + rc_inner) + 512))]));
        }
      }
    }
  }
  ...
  for (int ff_inner_inner_inner = 0; ff_inner_inner_inner < 2; ++ff_inner_inner_inner) {
    ...
    for (int yy_inner_inner_inner = 0; yy_inner_inner_inner < 7; ++yy_inner_inner_inner) {
      compute[((((((((int)blockIdx.z) * 6272) + (((int)threadIdx.z) * 98)) + (ff_inner_inner_inner * 49)) + (yy_inner_inner_inner * 7)) + ((int)threadIdx.x)))] = compute_local[(((ff_inner_inner_inner * 7) + yy_inner_inner_inner))];
      compute[(((((((((int)blockIdx.z) * 6272) + (((int)threadIdx.z) * 98)) + (ff_inner_inner_inner * 49)) + (yy_inner_inner_inner * 7)) + ((int)threadIdx.x)) + 3136))] = compute_local[((((ff_inner_inner_inner * 7) + yy_inner_inner_inner) + 14))];
    }
  }
}
