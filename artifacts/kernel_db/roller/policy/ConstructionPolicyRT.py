...
from roller.policy.PolicyBase import *
import math
import numpy
...
class ConstructionPolicyRT(PolicyBase):
    ...
    def EnlargeTile(self, last_rprog, rprog, steps, mem_level):
        ...
        if self.IsPeakComputeTile(rprog, mem_level) and self.IsComputeIntensive(rprog, mem_level):
            if mem_level == 0:
                if self.fix_block_size is not None and self.fix_block_size != rprog.GetParallelism(1):
                    print("ignore config due to fix_block_size", rprog.Dump(), "with block size", rprog.GetParallelism(1))
                    pass
                else:
    ...
    def emit_raw_configs(self, padding_threshold=0):
        ...
        uniProg = rProg(self.arch.num_level, self.op, self.fix_block_size)
