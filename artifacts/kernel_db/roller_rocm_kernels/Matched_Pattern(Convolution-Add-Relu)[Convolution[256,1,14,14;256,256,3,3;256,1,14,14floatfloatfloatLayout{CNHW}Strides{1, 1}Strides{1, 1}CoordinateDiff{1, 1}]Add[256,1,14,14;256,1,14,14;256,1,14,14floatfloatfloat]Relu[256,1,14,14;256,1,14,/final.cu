...
extern "C" __global__ void __launch_bounds__(128) default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ bias, float* __restrict__ conv_unpad) {
  ...
  for (int ra_fused0_outer = 0; ra_fused0_outer < 72; ++ra_fused0_outer) {
    __syncthreads();
    data_pad_shared[((int)threadIdx.x)] = (((((0 < (((((((int)blockIdx.x) % 49) * 2) + ((((int)threadIdx.x) & 3) >> 1)) / 7) + ((((ra_fused0_outer * 5) + (((int)threadIdx.x) >> 2)) % 9) / 3))) && ((((((((int)blockIdx.x) % 49) * 2) + ((((int)threadIdx.x) & 3) >> 1)) / 7) + ((((ra_fused0_outer * 5) + (((int)threadIdx.x) >> 2)) % 9) / 3)) < 15)) && (0 < (((((((int)blockIdx.x) % 49) * 4) + (((int)threadIdx.x) & 3)) % 14) + (((ra_fused0_outer * 2) + (((int)threadIdx.x) >> 2)) % 3)))) && ((((((((int)blockIdx.x) % 49) * 4) + (((int)threadIdx.x) & 3)) % 14) + (((ra_fused0_outer * 2) + (((int)threadIdx.x) >> 2)) % 3)) < 15)) ? data[(((((((((ra_fused0_outer * 32) + (((int)threadIdx.x) >> 2)) / 9) * 196) + (((((ra_fused0_outer * 5) + (((int)threadIdx.x) >> 2)) % 9) / 3) * 14)) + ((((int)blockIdx.x) % 49) * 4)) + (((int)threadIdx.x) & 3)) + (((ra_fused0_outer * 2) + (((int)threadIdx.x) >> 2)) % 3)) - 15)] : 0.000000e+00f);
