...
from ast_analyzer.python_std.typing import Dict, Set, List, TypeVar, Union, Optional, NDArray
from ast_analyzer.python_std.typing import Generator, Fun, Tuple, Iterable, Sized, File

from ast_analyzer.python_std.conversion import to_ast, ToNotEval
from ast_analyzer.python_std.intrinsic import Class
from ast_analyzer.python_std.intrinsic import ClassWithConstConstructor, ExceptionClass
from ast_analyzer.python_std.intrinsic import ClassWithReadOnceConstructor
from ast_analyzer.python_std.intrinsic import ConstFunctionIntr, FunctionIntr, UpdateEffect
from ast_analyzer.python_std.intrinsic import ConstMethodIntr, MethodIntr, AttributeIntr
from ast_analyzer.python_std.intrinsic import ReadEffect, ConstantIntr, UFunc
from ast_analyzer.python_std.intrinsic import ReadOnceMethodIntr
from ast_analyzer.python_std.intrinsic import ReadOnceFunctionIntr, ConstExceptionIntr
from ast_analyzer.python_std import interval
...
MODULES = {
    ...
    "torch": {
        ...
        "sigmoid": ConstFunctionIntr(),
        "relu": ConstFunctionIntr(),
        "mm": ConstFunctionIntr(),
        "matmul": ConstFunctionIntr(),
        "tanh": ConstFunctionIntr(),
        "shape": ConstFunctionIntr(),
        "size": ConstFunctionIntr(),
        "cat": ConstFunctionIntr(),
        "view": ConstFunctionIntr(),
        "transpose": ConstFunctionIntr(),
        "t": ConstFunctionIntr(),
        "chunk": ConstFunctionIntr(),
        "cat": ConstFunctionIntr(),
        "reshape": ConstFunctionIntr(),
        "permute": ConstFunctionIntr(),
        "mul": ConstFunctionIntr(),
        "clone": ConstFunctionIntr(),
        "randint": FunctionIntr(global_effects=True),
        "rand": FunctionIntr(global_effects=True),
        "sum": ConstFunctionIntr(),
        "float": ConstFunctionIntr(),
        "int": ConstFunctionIntr(),
        "int64": ConstFunctionIntr(),
        "max": ConstFunctionIntr(),
        "unsqueeze": ConstFunctionIntr(),
        "eq": ConstFunctionIntr(),
        "lt": ConstFunctionIntr(),
        "ge": ConstFunctionIntr(),
        "item": ConstFunctionIntr(),
        "bool": ConstFunctionIntr(),
        "double": ConstFunctionIntr(),
        "narrow": ConstFunctionIntr(),
        "fill_": FunctionIntr(),
        "masked_fill": ConstFunctionIntr(),
        "zeros_like": ConstFunctionIntr(),
        "scatter": ConstFunctionIntr(),
        "unsqueeze": ConstFunctionIntr(),
        "mean": ConstFunctionIntr(),
        "expand_as": ConstFunctionIntr(),
        "empty": ConstFunctionIntr(),
    },
    "F": {
        "pad": ConstFunctionIntr(),
        "softmax": ConstFunctionIntr(),
        "log_softmax": ConstFunctionIntr(),
        "cross_entropy": ConstFunctionIntr(),
    },
    "time": {
        "sleep": FunctionIntr(
