    ...
    .infershape([](std::shared_ptr<graph::GNode> gnode) -> void {
        ...
        */
        ...
        NNFUSION_CHECK(input_shape.size() == 3)
            << "input is expected to have 3 dimensions, got " << input_shape.size();
        ...
        NNFUSION_CHECK(gama_shape.size() == 1)
            << "gamma is expected to have 1 dimension, got " << gama_shape.size();
        NNFUSION_CHECK(gama_shape[0] == input_shape[2])
            << "Last dimension of gamma and input does not match";

        auto beta_shape = gnode->get_input_shape(3);
        NNFUSION_CHECK(beta_shape.size() == 1)
            << "beta is expected to have 1 dimension, got " << beta_shape.size();
        ...
        if (gnode->get_input_size() == 5)
        {
            ...
            NNFUSION_CHECK(bias_shape.size() == 1)
                << "bias is expected to have 1 dimension, got " << bias_shape.size();
