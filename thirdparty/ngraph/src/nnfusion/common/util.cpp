...
// void nnfusion::dump(ostream& out, const void* _data, size_t _size)
// {
...
// std::string nnfusion::to_lower(const std::string& s)
// {
//     std::string rc = s;
...
// std::string nnfusion::to_upper(const std::string& s)
// {
//     std::string rc = s;
...
// string nnfusion::trim(const string& s)
// {
//     string rc = s;
...
// vector<string> nnfusion::split(const string& src, char delimiter, bool do_trim)
// {
//     size_t pos;
//     string token;
//     size_t start = 0;
...
// size_t nnfusion::hash_combine(const std::vector<size_t>& list)
// {
//     size_t seed = 0;
//     for (size_t v : list)
//     {
...
void* nnfusion::aligned_alloc(size_t alignment, size_t size)
{
#ifdef __APPLE__
...
void nnfusion::aligned_free(void* p)
{
#ifdef __APPLE__
...
// size_t nnfusion::round_up(size_t size, size_t alignment)
// {
//     if (alignment == 0)
//     {
//         return size;
...
// nnfusion::FpropCache nnfusion::cache_fprop(std::shared_ptr<nnfusion::Function> fprop,
//                                        std::shared_ptr<nnfusion::Function> bprop)
// {
//     using namespace nnfusion;
...
//         if (!result)
//         {
//             throw nnfusion_error("Expected op::Result values for op::Result keys in node_param_map");
//         }
...
//         {
//             throw nnfusion_error("Expected op::Result in fprop->get_results()");
//         }
...
// void nnfusion::check_fp_values_isinf(const char* name, const float* array, size_t n)
// {
...
// void nnfusion::check_fp_values_isinf(const char* name, const double* array, size_t n)
// {
//     for (size_t i = 0; i < n; i++)
//     {
//         if (std::isinf(array[i]))
...
// void nnfusion::check_fp_values_isnan(const char* name, const float* array, size_t n)
// {
//     for (size_t i = 0; i < n; i++)
//     {
//         if (std::isinf(array[i]))
...
// void nnfusion::check_fp_values_isnan(const char* name, const double* array, size_t n)
// {
//     for (size_t i = 0; i < n; i++)
//     {
//         if (std::isinf(array[i]))
...
// template <typename T>
// T nnfusion::apply_permutation(T input, AxisVector order)
// {
...
// template AxisVector nnfusion::apply_permutation<AxisVector>(AxisVector input, AxisVector order);
// template Shape nnfusion::apply_permutation<Shape>(Shape input, AxisVector order);

nnfusion::AxisVector nnfusion::get_default_order(const Shape& shape)
{
    return get_default_order(shape.size());
}

AxisVector nnfusion::get_default_order(size_t rank)
{
...
// AxisVector nnfusion::get_permutation_to_default_order(const AxisVector& axis_order)
// {
